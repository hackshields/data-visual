...
class CI_Session_memcached_driver extends CI_Session_driver implements SessionHandlerInterface
{
    ...
    {
        if (!(isset($this->_memcached) && isset($this->_lock_key))) {
            return $this->_fail();
        }
        ...
        if ($this->_memcached->touch($key, $this->_config["expiration"]) || $this->_memcached->getResultCode() === Memcached::RES_NOTFOUND && $this->_memcached->set($key, $session_data, $this->_config["expiration"])) {
            return $this->_success;
        }
        return $this->_fail();
    }
    ...
    {
        if (isset($this->_memcached) && isset($this->_lock_key) && $this->_lock) {
