...
class CI_Profiler
{
    ...
    protected function _compile_benchmarks()
    {
        ...
        foreach ($this->CI->benchmark->marker as $key => $val) {
            if (preg_match("/(.+?)_end\$/i", $key, $match) && isset($this->CI->benchmark->marker[$match[1] . "_end"]) && isset($this->CI->benchmark->marker[$match[1] . "_start"])) {
    ...
    protected function _compile_queries()
    {
        ...
        foreach ($dbs as $name => $db) {
            $hide_queries = $this->_query_toggle_count < count($db->queries) ? " display:none" : "";
    ...
    protected function _compile_get()
    {
        ...
        if (count($_GET) === 0) {
            ...
            foreach ($_GET as $key => $val) {
                is_int($key) or htmlspecialchars($key, ENT_QUOTES, config_item("charset"));
                $val = is_array($val) || is_object($val) ? "<pre>" . htmlspecialchars(print_r($val, true), ENT_QUOTES, config_item("charset")) . "</pre>" : htmlspecialchars($val, ENT_QUOTES, config_item("charset"));
