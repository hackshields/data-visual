...
/**
    ...
    public function select($select = "*", $escape = NULL)
    {
        ...
        is_bool($escape) or foreach( $select as $val ) 
{
    $val = trim($val);
    if( $val !== "" ) 
    {
    ...
    public function join($table, $cond, $type = "", $escape = NULL)
    {
        ...
        is_bool($escape) or if( !$this->_has_operator($cond) ) 
{
...
else
{
    ...
    protected function _wh($qb_key, $key, $value = NULL, $type = "AND ", $escape = NULL)
    {
        ...
        is_bool($escape) or foreach( $key as $k => $v ) 
{
    ...
    else
    {
        ...
        {
            if( preg_match("/\\s*(!?=|<>|IS(?:\\s+NOT)?)\\s*\$/i", $k, $match, PREG_OFFSET_CAPTURE) ) 
            {
    ...
    protected function _like($field, $match = "", $type = "AND ", $side = "both", $not = "", $escape = NULL)
    {
        ...
        is_bool($escape) or $side = strtolower($side);
        foreach( $field as $k => $v ) 
        {
            ...
            if( $side === "none" ) 
            {
                $like_statement = (string) $prefix . " " . $k . " " . $not . " LIKE '" . $v . "'";
            }
            else
            {
                if( $side === "before" ) 
    ...
    public function group_end()
    {
        ...
        $where = array( "condition" => str_repeat(" ", $this->qb_where_group_count--) . ")", "escape" => false );
        $this->qb_where[] = $where;
        if( $this->qb_caching ) 
        {
            $this->qb_cache_where[] = $where;
    ...
    {
        is_bool($escape) or if( is_string($by) ) 
{
    ...
    public function order_by($orderby, $direction = "", $escape = NULL)
    {
        ...
        is_bool($escape) or if( $escape === false ) 
{
...
else
{
    ...
    {
        is_null($value) or empty($offset) or return $this;
    }
        ...
        is_bool($escape) or foreach( $key as $k => $v ) 
{
    ...
    public function count_all_results($table = "", $reset = true)
    {
        ...
        $result = ($this->qb_distinct === true || !empty($this->qb_groupby) ? $this->query($this->_count_string . $this->protect_identifiers("numrows") . "\nFROM (\n" . $this->_compile_select() . "\n) CI_count_all_results") : $this->query($this->_compile_select($this->_count_string . $this->protect_identifiers("numrows"))));
        if( $reset === true ) 
        {
            $this->_reset_select();
        }
    ...
    public function set_insert_batch($key, $value = "", $escape = NULL)
    {
        ...
        is_bool($escape) or $keys = array_keys($this->_object_to_array(current($key)));
        sort($keys);
        foreach( $key as $row ) 
        {
            $row = $this->_object_to_array($row);
            if( 0 < count(array_diff($keys, array_keys($row))) || 0 < count(array_diff(array_keys($row), $keys)) ) 
            {
                $this->qb_set[] = array(  );
                return NULL;
            }
        ...
        is_bool($escape) or foreach( $key as $k => $v ) 
{
    ...
    public function delete($table = "", $where = "", $limit = NULL, $reset_data = true)
    {
        ...
        else
        {
            if( is_array($table) ) 
            {
                empty($where) and foreach( $table as $single_table ) 
{
    ...
    {
        if( 0 < count($this->$qb_key) ) 
        {
            $i = 0;
            for( $c = count($this->$qb_key); $i < $c; $i++ ) 
            {
                ...
                $ci = 0;
                for( $cc = count($conditions); $ci < $cc; $ci++ ) 
                {
    ...
    {
        if( 0 < count($this->qb_groupby) ) 
        {
            $i = 0;
            for( $c = count($this->qb_groupby); $i < $c; $i++ ) 
            {
                ...
                $this->qb_groupby[$i] = ($this->qb_groupby[$i]["escape"] === false || $this->_is_literal($this->qb_groupby[$i]["field"]) ? $this->qb_groupby[$i]["field"] : $this->protect_identifiers($this->qb_groupby[$i]["field"]));
            }
    ...
    {
        if( is_array($this->qb_orderby) && 0 < count($this->qb_orderby) ) 
        {
            $i = 0;
            for( $c = count($this->qb_orderby); $i < $c; $i++ ) 
            {
    ...
    {
        $this->_reset_run(array( "qb_cache_select" => array(  ), "qb_cache_from" => array(  ), "qb_cache_join" => array(  ), "qb_cache_where" => array(  ), "qb_cache_groupby" => array(  ), "qb_cache_having" => array(  ), "qb_cache_orderby" => array(  ), "qb_cache_set" => array(  ), "qb_cache_exists" => array(  ), "qb_cache_no_escape" => array(  ) ));
        return $this;
    }
    ...
    protected function _merge_cache()
    {
        ...
        if( in_array("select", $this->qb_cache_exists, true) ) 
        {
        ...
        foreach( array_unique($this->qb_cache_exists) as $val ) 
        {
            ...
            $i = 0;
            for( $c = count($this->$qb_variable); $i < $c; $i++ ) 
            {
        ...
        if( $this->_protect_identifiers === true && 0 < count($this->qb_cache_from) ) 
        {
    ...
    {
        $str = trim($str);
        if( empty($str) || ctype_digit($str) || (string) (double) $str === $str || in_array(strtoupper($str), array( "TRUE", "FALSE" ), true) ) 
        {
            return true;
        }
