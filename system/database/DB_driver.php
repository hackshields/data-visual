...
abstract class CI_DB_driver
{
    ...
    public function query($sql, $binds = false, $return_object = NULL)
    {
        ...
        if ($this->cache_on === true && $return_object === true && $this->_cache_init() && false !== ($cache = $this->CACHE->read($sql))) {
            return $cache;
        }
    ...
    {
        empty($this->conn_id) and $this->initialize();
    ...
    public function trans_commit()
    {
        ...
        if (1 < $this->_trans_depth || $this->_trans_commit()) {
            $this->_trans_depth--;
            return true;
        }
        return false;
    ...
    public function trans_rollback()
    {
        ...
        if (1 < $this->_trans_depth || $this->_trans_rollback()) {
            $this->_trans_depth--;
            return true;
        }
        return false;
    ...
    public function escape_identifiers($item)
    {
        ...
        if (is_array($item)) {
            ...
            if (ctype_digit($item) || $item[0] === "'" || $this->_escape_char !== "\"" && $item[0] === "\"" || strpos($item, "(") !== false) {
                return $item;
            }
            ...
            if (empty($preg_ec)) {
                if (is_array($this->_escape_char)) {
                    ...
                    $preg_ec[1] = preg_quote($this->_escape_char, "/");
                    $preg_ec[0] = $preg_ec[1];
                    $preg_ec[3] = $this->_escape_char;
                    $preg_ec[2] = $preg_ec[3];
                }
            }
    ...
    protected function _get_operator($str)
    {
        ...
        if (empty($_operators)) {
            ...
            $_operators = array("\\s*(?:<|>|!)?=\\s*", "\\s*<>?\\s*", "\\s*>\\s*", "\\s+IS NULL", "\\s+IS NOT NULL", "\\s+EXISTS\\s*\\(.*\\)", "\\s+NOT EXISTS\\s*\\(.*\\)", "\\s+BETWEEN\\s+", "\\s+IN\\s*\\(.*\\)", "\\s+NOT IN\\s*\\(.*\\)", "\\s+LIKE\\s+\\S.*(" . $_les . ")?", "\\s+NOT LIKE\\s+\\S.*(" . $_les . ")?");
        }
    ...
    public function call_function($function)
    {
        ...
        return 1 < func_num_args() ? call_user_func_array($function, array_slice(func_get_args(), 1)) : call_user_func($function);
    }
    /**
    ...
    public function cache_set_ttl($ttl)
    {
        $this->cache_ttl = $ttl;
    }
    /**
    ...
    public function display_error($error = "", $swap = "", $native = false)
    {
        ...
        foreach ($trace as $call) {
            if (isset($call["file"]) && isset($call["class"])) {
            ...
            if (strpos($item, ".") !== false) {
                ...
                if (!empty($this->qb_aliased_tables) && in_array($parts[0], $this->qb_aliased_tables)) {
                    ...
                    if ($this->dbprefix !== "") {
                        ...
                        if ($this->swap_pre !== "" && strpos($parts[$i], $this->swap_pre) === 0) {
                            ...
                            if (strpos($parts[$i], $this->dbprefix) !== 0) {
                ...
                if ($this->dbprefix !== "") {
                    if ($this->swap_pre !== "" && strpos($item, $this->swap_pre) === 0) {
                        ...
                        if ($prefix_single === true && strpos($item, $this->dbprefix) !== 0) {
