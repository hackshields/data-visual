...
/**
    ...
    public function create_database($db_name)
    {
        ...
        if( !$this->db->query(sprintf($this->_create_database, $db_name, $this->db->char_set, $this->db->dbcollat)) ) 
        {
    ...
    public function drop_database($db_name)
    {
        ...
        if( !$this->db->query(sprintf($this->_drop_database, $db_name)) ) 
        {
            return ($this->db->db_debug ? $this->db->display_error("db_unable_to_drop") : false);
        }
    ...
    {
        is_array($field) or foreach( array_keys($field) as $k ) 
{
        ...
        is_array($field) or foreach( array_keys($field) as $k ) 
{
    ...
    protected function _process_fields($create_table = false)
    {
        ...
        foreach( $this->fields as $key => $attributes ) 
        {
            ...
            isset($attributes["TYPE"]) and $this->_attr_type($attributes);
            ...
            isset($attributes["TYPE"]) and $this->_attr_unsigned($attributes, $field);
