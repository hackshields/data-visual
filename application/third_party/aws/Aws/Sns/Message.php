...
{
    private static $requiredKeys = ['Message', 'MessageId', 'Timestamp', 'TopicArn', 'Type', 'Signature', ['SigningCertURL', 'SigningCertUrl'], 'SignatureVersion'];
    private static $subscribeKeys = [['SubscribeURL', 'SubscribeUrl'], 'Token'];
    ...
    public function __construct(array $data)
    {
        ...
        if ($data['Type'] === 'SubscriptionConfirmation' || $data['Type'] === 'UnsubscribeConfirmation') {
    ...
    private function validateRequiredKeys(array $data, array $keys)
    {
        foreach ($keys as $key) {
            ...
            if (!$found) {
                ...
                throw new \InvalidArgumentException("\"{$key}\" is required to verify the SNS Message.");
            }
        }
    }
}
