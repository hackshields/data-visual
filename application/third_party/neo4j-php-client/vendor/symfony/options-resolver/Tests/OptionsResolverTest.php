...
class OptionsResolverTest extends TestCase
{
    ...
    {
        $this->resolver->setDefault('bar', 'baz')->setDefault('foo', function (Options $options) {
    ...
    {
        $this->resolver->setDefined('foo')->setDeprecated('foo', true);
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidArgumentException
    ...
    {
        $this->resolver->setDefined('foo')->setDeprecated('foo', function (Options $options, $value) {
            return false;
        });
    ...
    {
        $this->resolver->setDefined(array('foo', 'bar'))->setDeprecated('foo', function (Options $options, $value) {
            $options['bar'];
    ...
    {
        $this->resolver->setDefined('foo')->setDeprecated('foo');
    ...
    {
        (yield 'It deprecates an option with default message' => array(function (OptionsResolver $resolver) {
            $resolver->setDefined(array('foo', 'bar'))->setDeprecated('foo');
        }, array('foo' => 'baz'), array('type' => E_USER_DEPRECATED, 'message' => 'The option "foo" is deprecated.'), 1));
        (yield 'It deprecates an option with custom message' => array(function (OptionsResolver $resolver) {
            $resolver->setDefined('foo')->setDefault('bar', function (Options $options) {
                return $options['foo'];
            })->setDeprecated('foo', 'The option "foo" is deprecated, use "bar" option instead.');
        }, array('foo' => 'baz'), array('type' => E_USER_DEPRECATED, 'message' => 'The option "foo" is deprecated, use "bar" option instead.'), 2));
        (yield 'It deprecates an option evaluated in another definition' => array(function (OptionsResolver $resolver) {
            // defined by superclass
            $resolver->setDefault('foo', null)->setDeprecated('foo');
            // defined by subclass
            $resolver->setDefault('bar', function (Options $options) {
                return $options['foo'];
                // It triggers a deprecation
            });
        }, array(), array('type' => E_USER_DEPRECATED, 'message' => 'The option "foo" is deprecated.'), 1));
        (yield 'It deprecates allowed type and value' => array(function (OptionsResolver $resolver) {
            $resolver->setDefault('foo', null)->setAllowedTypes('foo', array('null', 'string', \stdClass::class))->setDeprecated('foo', function (Options $options, $value) {
                if ($value instanceof \stdClass) {
                    return sprintf('Passing an instance of "%s" to option "foo" is deprecated, pass its FQCN instead.', \stdClass::class);
                }
                return '';
            });
        }, array('foo' => new \stdClass()), array('type' => E_USER_DEPRECATED, 'message' => 'Passing an instance of "stdClass" to option "foo" is deprecated, pass its FQCN instead.'), 1));
        (yield 'It triggers a deprecation based on the value only if option is provided by the user' => array(
            function (OptionsResolver $resolver) {
                $resolver->setDefined('foo')->setAllowedTypes('foo', array('null', 'bool'))->setDeprecated('foo', function (Options $options, $value) {
                    if (!\is_bool($value)) {
                        return 'Passing a value different than true or false is deprecated.';
                    }
                    return '';
                })->setDefault('baz', null)->setAllowedTypes('baz', array('null', 'int'))->setDeprecated('baz', function (Options $options, $value) {
                    if (!\is_int($value)) {
                    ...
                    $options['baz'];
                    // It does not triggers a deprecation
                    return $options['foo'];
                    // It does not triggers a deprecation
                });
            },
            array('foo' => null),
            // It triggers a deprecation
            array('type' => E_USER_DEPRECATED, 'message' => 'Passing a value different than true or false is deprecated.'),
            1,
        ));
        (yield 'It ignores a deprecation if closure returns an empty string' => array(function (OptionsResolver $resolver) {
            $resolver->setDefault('foo', null)->setDeprecated('foo', function (Options $options, $value) {
                return '';
            });
        }, array('foo' => Bar::class), null, 0));
        (yield 'It deprecates value depending on other option value' => array(function (OptionsResolver $resolver) {
            $resolver->setDefault('widget', null)->setDefault('date_format', null)->setDeprecated('date_format', function (Options $options, $dateFormat) {
        ...
        }, array('widget' => 'single_text', 'date_format' => 2), array('type' => E_USER_DEPRECATED, 'message' => 'Using the "date_format" option when the "widget" option is set to "single_text" is deprecated.'), 1));
        (yield 'It triggers a deprecation for each evaluation' => array(
            function (OptionsResolver $resolver) {
                $resolver->setDefined('foo')->setDeprecated('foo')->setDefault('bar', function (Options $options) {
                    return $options['foo'];
                    // It triggers a deprecation
            ...
            array('foo' => 'baz'),
            // It triggers a deprecation
            array('type' => E_USER_DEPRECATED, 'message' => 'The option "foo" is deprecated.'),
            4,
        ));
        (yield 'It ignores a deprecation if no option is provided by the user' => array(function (OptionsResolver $resolver) {
            $resolver->setDefined('foo')->setDefault('bar', null)->setDeprecated('foo')->setDeprecated('bar');
        }, array(), null, 0));
        (yield 'It explicitly ignores a depreciation' => array(function (OptionsResolver $resolver) {
            $resolver->setDefault('baz', function (Options $options) {
                return $options->offsetGet('foo', false);
            })->setDefault('foo', null)->setDeprecated('foo')->setDefault('bar', function (Options $options) {
                return $options->offsetGet('foo', false);
            });
        }, array(), null, 0));
    }
    ...
    public function testResolveFailsWithCorrectLevelsButWrongScalar()
    {
        $this->resolver->setDefined('foo');
        $this->resolver->setAllowedTypes('foo', 'int[][]');
        $this->resolver->resolve(array('foo' => array(array(1.2))));
    }
    /**
    ...
    public function testResolveSucceedsIfTypedArray()
    {
        ...
        $data = array('foo' => array(new \DateTime(), new \DateTime()));
    ...
    public function testNestedArrays()
    {
        $this->resolver->setDefined('foo');
        $this->resolver->setAllowedTypes('foo', 'int[][]');
        $this->assertEquals(array('foo' => array(array(1, 2))), $this->resolver->resolve(array('foo' => array(array(1, 2)))));
    }
    public function testNested2Arrays()
    {
        $this->resolver->setDefined('foo');
        $this->resolver->setAllowedTypes('foo', 'int[][][][]');
        $this->assertEquals(array('foo' => array(array(array(array(1, 2))))), $this->resolver->resolve(array('foo' => array(array(array(array(1, 2)))))));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
    ...
    public function testNestedArraysException()
    {
        ...
        $this->resolver->resolve(array('foo' => array(array(array(array(1, 2))))));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
    ...
    public function testNestedArrayException1()
    {
        $this->resolver->setDefined('foo');
        $this->resolver->setAllowedTypes('foo', 'int[][]');
        $this->resolver->resolve(array('foo' => array(array(1, true, 'str', array(2, 3)))));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
     * @expectedExceptionMessage The option "foo" with value array is expected to be of type "int[][]", but one of the elements is of type "boolean[][]".
    ...
    public function testNestedArrayException2()
    {
        $this->resolver->setDefined('foo');
        $this->resolver->setAllowedTypes('foo', 'int[][]');
        $this->resolver->resolve(array('foo' => array(array(true, 'str', array(2, 3)))));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
    ...
    public function testNestedArrayException3()
    {
        $this->resolver->setDefined('foo');
        $this->resolver->setAllowedTypes('foo', 'string[][][]');
        $this->resolver->resolve(array('foo' => array(array('str', array(1, 2)))));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
    ...
    public function testNestedArrayException4()
    {
        $this->resolver->setDefined('foo');
        $this->resolver->setAllowedTypes('foo', 'string[][][]');
        $this->resolver->resolve(array('foo' => array(array(array('str'), array(1, 2)))));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
    ...
    {
        $this->resolver->setDefaults(array('database' => function (OptionsResolver $resolver) {
    ...
    {
        $this->resolver->setDefaults(array('name' => 'default', 'database' => function (OptionsResolver $resolver) {
            $resolver->setDefined(array('host', 'port'));
        }));
        $this->resolver->resolve(array('database' => array('foo' => 'bar')));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\MissingOptionsException
    ...
    public function testFailsIfMissingRequiredNestedOption()
    {
        ...
        $this->resolver->resolve(array('database' => array()));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
    ...
    {
        $this->resolver->setDefaults(array('name' => 'default', 'database' => function (OptionsResolver $resolver) {
            $resolver->setDefined('logging')->setAllowedTypes('logging', 'bool');
        }));
        $this->resolver->resolve(array('database' => array('logging' => null)));
    }
    /**
     * @expectedException \Symfony\Component\OptionsResolver\Exception\InvalidOptionsException
    ...
    public function testFailsIfNotArrayIsGivenForNestedOptions()
    {
        ...
        $this->resolver->resolve(array('database' => null));
    }
    public function testResolveNestedOptionsWithoutDefault()
    {
        ...
        $expectedOptions = array('name' => 'default', 'database' => array());
        $this->assertSame($expectedOptions, $actualOptions);
    }
    public function testResolveNestedOptionsWithDefault()
    {
        $this->resolver->setDefaults(array('name' => 'default', 'database' => function (OptionsResolver $resolver) {
            $resolver->setDefaults(array('host' => 'localhost', 'port' => 3306));
        }));
        $actualOptions = $this->resolver->resolve();
        $expectedOptions = array('name' => 'default', 'database' => array('host' => 'localhost', 'port' => 3306));
        $this->assertSame($expectedOptions, $actualOptions);
    }
    public function testResolveMultipleNestedOptions()
    {
        $this->resolver->setDefaults(array('name' => 'default', 'database' => function (OptionsResolver $resolver) {
            $resolver->setRequired(array('dbname', 'host'))->setDefaults(array('port' => 3306, 'replicas' => function (OptionsResolver $resolver) {
                $resolver->setDefaults(array('host' => 'replica1', 'port' => 3306));
            }));
        }));
        $actualOptions = $this->resolver->resolve(array('name' => 'custom', 'database' => array('dbname' => 'test', 'host' => 'localhost', 'port' => null, 'replicas' => array('host' => 'replica2'))));
        $expectedOptions = array('name' => 'custom', 'database' => array('port' => null, 'replicas' => array('port' => 3306, 'host' => 'replica2'), 'dbname' => 'test', 'host' => 'localhost'));
        $this->assertSame($expectedOptions, $actualOptions);
    }
    public function testResolveLazyOptionUsingNestedOption()
    {
        $this->resolver->setDefaults(array('version' => function (Options $options) {
            return $options['database']['server_version'];
        }, 'database' => function (OptionsResolver $resolver) {
            $resolver->setDefault('server_version', '3.15');
        }));
        $actualOptions = $this->resolver->resolve();
        $expectedOptions = array('database' => array('server_version' => '3.15'), 'version' => '3.15');
        $this->assertSame($expectedOptions, $actualOptions);
    }
    public function testNormalizeNestedOptionValue()
    {
        $this->resolver->setDefaults(array('database' => function (OptionsResolver $resolver) {
            $resolver->setDefaults(array('port' => 3306, 'host' => 'localhost', 'dbname' => 'demo'));
        }))->setNormalizer('database', function (Options $options, $value) {
            ksort($value);
            return $value;
        });
        $actualOptions = $this->resolver->resolve(array('database' => array('dbname' => 'test')));
        $expectedOptions = array('database' => array('dbname' => 'test', 'host' => 'localhost', 'port' => 3306));
        $this->assertSame($expectedOptions, $actualOptions);
    }
    ...
    {
        $this->resolver->setDefault('name', 'default')->setDefault('database', function (OptionsResolver $resolver, Options $parent) {
    ...
    {
        $this->resolver->setDefaults(array('version' => 3.15, 'database' => function (OptionsResolver $resolver, Options $parent) {
            $resolver->setDefault('server_version', $parent['version']);
        }));
    ...
    {
        $this->resolver->setDefaults(array('ip' => null, 'database' => function (OptionsResolver $resolver, Options $parent) {
        ...
        $expectedOptions = array('ip' => '127.0.0.1', 'database' => array('host' => '127.0.0.1', 'primary_replica' => array('host' => '127.0.0.1')), 'secondary_replica' => '127.0.0.1');
        $this->assertSame($expectedOptions, $actualOptions);
    }
    public function testAccessToParentOptionFromNestedNormalizerAndLazyOption()
    {
        $this->resolver->setDefaults(array('debug' => true, 'database' => function (OptionsResolver $resolver, Options $parent) {
            $resolver->setDefined('logging')->setDefault('profiling', function (Options $options) use($parent) {
                return $parent['debug'];
            })->setNormalizer('logging', function (Options $options, $value) use($parent) {
                return false === $parent['debug'] ? true : $value;
            });
        }));
        $actualOptions = $this->resolver->resolve(array('debug' => false, 'database' => array('logging' => false)));
        $expectedOptions = array('debug' => false, 'database' => array('profiling' => false, 'logging' => true));
        $this->assertSame($expectedOptions, $actualOptions);
    }
}
