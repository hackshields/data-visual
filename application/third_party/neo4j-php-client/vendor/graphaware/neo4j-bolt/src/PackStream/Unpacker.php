...
class Unpacker
{
    ...
    public function unpackElement(BytesWalker $walker)
    {
        ...
        if ($this->isInRange(0x0, 0x7f, $marker) || $this->isInRange(0xf0, 0xff, $marker)) {
            $walker->rewind(1);
            $integer = $this->readSignedShortShort($walker);
            return $this->unpackInteger($integer);
        }
    ...
    {
        static $signatures = [Constants::SIGNATURE_SUCCESS => self::SUCCESS, Constants::SIGNATURE_FAILURE => self::FAILURE, Constants::SIGNATURE_RECORD => self::RECORD, Constants::SIGNATURE_IGNORE => self::IGNORED, Constants::SIGNATURE_UNBOUND_RELATIONSHIP => 'UNBOUND_RELATIONSHIP', Constants::SIGNATURE_NODE => 'NODE', Constants::SIGNATURE_PATH => 'PATH', Constants::SIGNATURE_RELATIONSHIP => 'RELATIONSHIP'];
    ...
    {
        return (bool) ($longInt & 2147483648.0);
    }
}
