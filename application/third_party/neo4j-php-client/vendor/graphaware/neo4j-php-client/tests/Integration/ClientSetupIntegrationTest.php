...
class ClientSetupIntegrationTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $client = ClientBuilder::create()->addConnection('default', 'http://localhost:7474')->addConnection('conn2', 'http://localhost:7575')->addConnection('conn3', 'http://localhost:7676')->setMaster('conn2')->build();
    ...
    {
        $client = ClientBuilder::create()->addConnection('default', 'http://localhost:7474')->addConnection('conn2', 'http://localhost:7575')->addConnection('conn3', 'http://localhost:7676')->setMaster('conn2')->setMaster('default')->build();
    ...
    public function testSendWriteUseMasterIfAvailable()
    {
        ...
        if (isset($_ENV['NEO4J_USER'])) {
            $httpUri = sprintf('%s://%s:%s@%s:%s', getenv('NEO4J_SCHEMA'), getenv('NEO4J_USER'), getenv('NEO4J_PASSWORD'), getenv('NEO4J_HOST'), getenv('NEO4J_PORT'));
        }
