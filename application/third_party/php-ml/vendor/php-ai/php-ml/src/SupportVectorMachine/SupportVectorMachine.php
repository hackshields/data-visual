...
class SupportVectorMachine
{
    ...
    private function runSvmPredict(array $samples, bool $probabilityEstimates) : string
    {
        ...
        $command = $this->buildPredictCommand($testSetFileName, $modelFileName, $outputFileName, $probabilityEstimates);
        $output = [];
        exec(escapeshellcmd($command) . ' 2>&1', $output, $return);
    ...
    {
        return sprintf('%ssvm-predict%s -b %d %s %s %s', $this->binPath, $this->getOSExtension(), $probabilityEstimates ? 1 : 0, escapeshellarg($testSetFileName), escapeshellarg($modelFileName), escapeshellarg($outputFileName));
    }
