...
class DecisionStump extends WeightedClassifier
{
    ...
    {
        return "IF {$this}->column {$this}->operator {$this}->value " . 'THEN ' . $this->binaryLabels[0] . ' ' . 'ELSE ' . $this->binaryLabels[1];
    }
    /**
    ...
    protected function trainBinary(array $samples, array $targets, array $labels) : void
    {
        ...
        $bestSplit = ['value' => 0, 'operator' => '', 'prob' => [], 'column' => 0, 'trainingErrorRate' => 1.0];
    ...
    protected function getBestNumericalSplit(array $samples, array $targets, int $col) : array
    {
        ...
        foreach (['<=', '>'] as $operator) {
            ...
            $threshold = array_sum($values) / (double) count($values);
            [$errorRate, $prob] = $this->calculateErrorRate($targets, $threshold, $operator, $values);
            if ($split === [] || $errorRate < $split['trainingErrorRate']) {
                $split = ['value' => $threshold, 'operator' => $operator, 'prob' => $prob, 'column' => $col, 'trainingErrorRate' => $errorRate];
            }
            // Try other possible points one by one
            for ($step = $minValue; $step <= $maxValue; $step += $stepSize) {
                $threshold = (double) $step;
                [$errorRate, $prob] = $this->calculateErrorRate($targets, $threshold, $operator, $values);
        ...
        foreach (['=', '!='] as $operator) {
            foreach ($distinctVals as $val) {
                ...
                if ($split === [] || $split['trainingErrorRate'] < $errorRate) {
                    $split = ['value' => $val, 'operator' => $operator, 'prob' => $prob, 'column' => $col, 'trainingErrorRate' => $errorRate];
                }
            }
        }
        return $split;
    ...
    protected function calculateErrorRate(array $targets, float $threshold, string $operator, array $values) : array
    {
        ...
        foreach ($prob as $leaf => $counts) {
            $leafTotal = (double) array_sum($prob[$leaf]);
        ...
        return [$wrong / (double) array_sum($this->weights), $dist];
    }
    /**
