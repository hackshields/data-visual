...
class Dataset
{
     ...
     */
    public function __construct(ConnectionInterface $connection, $id, $projectId, ValueMapper $mapper, array $info = [], $location = null)
    {
    ...
    {
        $this->connection->deleteDataset($options + ['retries' => 0] + $this->identity);
    }
    /**
     * Update the dataset.
     *
    ...
    {
        return new Table($this->connection, $id, $this->identity['datasetId'], $this->identity['projectId'], $this->mapper, [], isset($this->info['location']) ? $this->info['location'] : $this->location);
    }
    /**
    ...
    public function tables(array $options = [])
    {
        ...
        return new ItemIterator(new PageIterator(function (array $table) {
            return new Table($this->connection, $table['tableReference']['tableId'], $this->identity['datasetId'], $this->identity['projectId'], $this->mapper, $table);
        }, [$this->connection, 'listTables'], $options + $this->identity, ['itemsKey' => 'tables', 'resultLimit' => $resultLimit]));
    }
    /**
     * Creates a table.
     *
    ...
    public function createTable($id, array $options = [])
    {
        ...
        $response = $this->connection->insertTable(['projectId' => $this->identity['projectId'], 'datasetId' => $this->identity['datasetId'], 'tableReference' => $this->identity + ['tableId' => $id]] + $options + ['retries' => 0]);
        return new Table($this->connection, $id, $this->identity['datasetId'], $this->identity['projectId'], $this->mapper, $response);
    }
    /**
