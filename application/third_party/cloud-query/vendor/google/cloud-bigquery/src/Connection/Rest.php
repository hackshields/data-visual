...
class Rest implements ConnectionInterface
{
    ...
    {
        $config += ['serviceDefinitionPath' => __DIR__ . '/ServiceDefinition/bigquery-v2.json', 'componentVersion' => BigQueryClient::VERSION];
        $this->setRequestWrapper(new RequestWrapper($config));
        $this->setRequestBuilder(new RequestBuilder($config['serviceDefinitionPath'], self::BASE_URI));
    }
    /**
     * @param array $args
     * @return array
    ...
    {
        $args += ['projectId' => null, 'data' => null, 'configuration' => [], 'labels' => [], 'dryRun' => false, 'jobReference' => []];
        $args['data'] = Psr7\stream_for($args['data']);
        $args['metadata'] = $this->pluckArray(['labels', 'dryRun', 'jobReference', 'configuration'], $args);
        $uploaderOptionKeys = ['restOptions', 'retries', 'requestTimeout', 'metadata'];
