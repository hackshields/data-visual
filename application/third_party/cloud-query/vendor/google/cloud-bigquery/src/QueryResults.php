...
class QueryResults implements \IteratorAggregate
{
     ...
     *        get query results.
     */
    public function __construct(ConnectionInterface $connection, $jobId, $projectId, array $info, ValueMapper $mapper, Job $job, array $queryResultsOptions = [])
    {
        $this->connection = $connection;
        $this->info = $info;
        $this->job = $job;
        $this->identity = ['jobId' => $jobId, 'projectId' => $projectId, 'location' => isset($info['jobReference']['location']) ? $info['jobReference']['location'] : $job->identity()['location']];
        $this->mapper = $mapper;
    ...
    public function rows(array $options = [])
    {
        ...
        return new ItemIterator(new PageIterator(function (array $row) use($schema) {
            ...
            return $mergedRow;
        }, [$this->connection, 'getQueryResults'], $options + $this->identity, ['itemsKey' => 'rows', 'firstPage' => $this->info, 'nextResultTokenKey' => 'pageToken']));
    }
    /**
    ...
    {
        return $this->info = $this->connection->getQueryResults($options + $this->identity);
    }
    /**
