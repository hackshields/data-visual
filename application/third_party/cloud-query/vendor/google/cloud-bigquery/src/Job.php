...
class Job
{
    ...
    public function __construct(ConnectionInterface $connection, $id, $projectId, ValueMapper $mapper, array $info = [], $location = null)
    {
        ...
        $this->info = $info;
        $this->identity = ['jobId' => $id, 'projectId' => $projectId, 'location' => isset($info['jobReference']['location']) ? $info['jobReference']['location'] : $location];
        $this->mapper = $mapper;
    }
    /**
    ...
    {
        $this->connection->cancelJob($options + ['retries' => 0] + $this->identity);
    }
    /**
    ...
    public function queryResults(array $options = [])
    {
        ...
        return new QueryResults($this->connection, $this->identity['jobId'], $this->identity['projectId'], $this->connection->getQueryResults($options + $this->identity), $this->mapper, $this, $queryResultsOptions);
    }
    /**
    ...
    public function waitUntilComplete(array $options = [])
    {
        ...
        $this->wait(function () use($options) {
