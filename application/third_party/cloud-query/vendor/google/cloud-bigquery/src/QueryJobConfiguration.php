...
class QueryJobConfiguration implements JobConfigurationInterface
{
     ...
     *        executed.
     */
    public function __construct(ValueMapper $mapper, $projectId, array $config, $location)
    {
        $this->mapper = $mapper;
    ...
    {
        $queryParams = [];
        $this->config['configuration']['query']['parameterMode'] = $this->isAssoc($parameters) ? 'named' : 'positional';
