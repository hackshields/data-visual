...
class Table
{
     ...
     */
    public function __construct(ConnectionInterface $connection, $id, $datasetId, $projectId, ValueMapper $mapper, array $info = [], $location = null)
    {
        $this->connection = $connection;
        $this->info = $info;
        $this->mapper = $mapper;
        $this->identity = ['tableId' => $id, 'datasetId' => $datasetId, 'projectId' => $projectId];
        $this->location = $location;
        $this->setHttpRetryCodes([502]);
        $this->setHttpRetryMessages(['rateLimitExceeded', 'backendError']);
    }
    /**
    ...
    {
        $this->connection->deleteTable($options + ['retries' => 0] + $this->identity);
    }
    /**
     * Update the table.
     *
    ...
    {
        $options = $this->applyEtagHeader($options + $metadata + $this->identity);
    ...
    public function rows(array $options = [])
    {
        ...
        return new ItemIterator(new PageIterator(function (array $row) use($schema) {
            $mergedRow = [];
            if ($row === null) {
                return $mergedRow;
            }
            ...
            return $mergedRow;
        }, [$this->connection, 'listTableData'], $options + $this->identity, ['nextResultTokenKey' => 'pageToken', 'itemsKey' => 'rows', 'resultLimit' => $resultLimit]));
    }
    /**
    ...
    public function startJob(JobConfigurationInterface $config, array $options = [])
    {
        ...
        return new Job($this->connection, $config['jobReference']['jobId'], $this->identity['projectId'], $this->mapper, $response);
    }
    /**
    ...
    {
        return (new CopyJobConfiguration($this->identity['projectId'], $options, isset($this->info['location']) ? $this->info['location'] : $this->location))->destinationTable($destination)->sourceTable($this);
    }
    /**
    ...
    public function extract($destination, array $options = [])
    {
        ...
        return (new ExtractJobConfiguration($this->identity['projectId'], $options, isset($this->info['location']) ? $this->info['location'] : $this->location))->destinationUris([$destination])->sourceTable($this);
    }
    /**
    ...
    {
        $config = (new LoadJobConfiguration($this->identity['projectId'], $options, isset($this->info['location']) ? $this->info['location'] : $this->location))->destinationTable($this);
        if ($data) {
            $config->data($data);
        }
        return $config;
    ...
    public function insertRows(array $rows, array $options = [])
    {
        ...
        return new InsertResponse($this->handleInsert($options), $options['rows']);
    }
    /**
    ...
    private function handleInsert(array $options)
    {
        ...
        while (true) {
            try {
                return $this->connection->insertAllTableData($this->identity + $options);
            } catch (NotFoundException $ex) {
                if ($autoCreate === true && $attempt <= $maxRetries) {
                    if (!isset($metadata['schema'])) {
                        throw new \InvalidArgumentException('A schema is required when creating a table.');
                    }
                    $this->usleep(mt_rand(1, self::INSERT_CREATE_MAX_DELAY_MICROSECONDS));
                    try {
                        $this->connection->insertTable($metadata + ['projectId' => $this->identity['projectId'], 'datasetId' => $this->identity['datasetId'], 'tableReference' => $this->identity, 'retries' => 0]);
