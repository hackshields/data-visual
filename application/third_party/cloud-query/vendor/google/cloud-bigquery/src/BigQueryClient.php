...
class BigQueryClient
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $config += ['scopes' => [self::SCOPE], 'projectIdRequired' => true, 'returnInt64AsObject' => false, 'restRetryFunction' => $this->getRetryFunction(), 'restDelayFunction' => function ($attempt) {
    ...
    {
        return (new QueryJobConfiguration($this->mapper, $this->projectId, $options, $this->location))->query($query);
    }
    /**
     * Returns a job configuration to be passed to either
     * {@see Google\Cloud\BigQuery\BigQueryClient::runQuery()} or
    ...
    {
        $queryResultsOptions = $this->pluckArray(['maxResults', 'startIndex', 'timeoutMs', 'maxRetries'], $options);
        $queryResultsOptions['initialTimeoutMs'] = 10000;
        $queryResults = $this->startQuery($query, $options)->queryResults($queryResultsOptions + $options);
    ...
    public function startQuery(JobConfigurationInterface $query, array $options = [])
    {
        ...
        return new Job($this->connection, $config['jobReference']['jobId'], $this->projectId, $this->mapper, $response);
    }
    /**
    ...
    {
        return new Job($this->connection, $id, $this->projectId, $this->mapper, [], isset($options['location']) ? $options['location'] : $this->location);
    }
    /**
    ...
    public function jobs(array $options = [])
    {
        ...
        return new ItemIterator(new PageIterator(function (array $job) {
            return new Job($this->connection, $job['jobReference']['jobId'], $this->projectId, $this->mapper, $job);
        }, [$this->connection, 'listJobs'], $options + ['projectId' => $this->projectId], ['itemsKey' => 'jobs', 'resultLimit' => $resultLimit]));
    }
    /**
    ...
    {
        return new Dataset($this->connection, $id, $this->projectId, $this->mapper, [], $this->location);
    }
    /**
    ...
    {
        $resultLimit = $this->pluck('resultLimit', $options, false);
        return new ItemIterator(new PageIterator(function (array $dataset) {
            return new Dataset($this->connection, $dataset['datasetReference']['datasetId'], $this->projectId, $this->mapper, $dataset);
        }, [$this->connection, 'listDatasets'], $options + ['projectId' => $this->projectId], ['itemsKey' => 'datasets', 'resultLimit' => $resultLimit]));
    }
    /**
     * Creates a dataset.
     *
    ...
    public function createDataset($id, array $options = [])
    {
        ...
        $response = $this->connection->insertDataset(['projectId' => $this->projectId, 'datasetReference' => ['datasetId' => $id]] + $options + ['retries' => 0]);
        return new Dataset($this->connection, $id, $this->projectId, $this->mapper, $response);
    }
    /**
