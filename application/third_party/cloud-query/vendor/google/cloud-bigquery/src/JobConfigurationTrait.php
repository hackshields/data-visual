...
trait JobConfigurationTrait
{
     ...
     *        executed.
     */
    public function jobConfigurationProperties($projectId, array $config, $location)
    {
        $this->config = array_replace_recursive(['projectId' => $projectId, 'jobReference' => ['projectId' => $projectId]], $config);
    ...
    {
        if ($this->jobIdPrefix) {
            $this->config['jobReference']['jobId'] = sprintf('%s-%s', $this->jobIdPrefix, $this->config['jobReference']['jobId']);
        }
        return $this->config;
    }
    /**
