...
class SysVCacheItemPoolTest extends TestCase
{
    ...
    public function setUp()
    {
        if (!extension_loaded('sysvshm')) {
            $this->markTestSkipped('sysvshm extension is required for running the test');
        }
    ...
    public function testCacheAmongProcesses()
    {
        ...
        $this->assertEquals($expectedValue, $this->pool->getItem('separate-process-item')->get());
    }
    ...
    public function testCommitsDeferredItems()
    {
        ...
        $this->assertEquals($item->get(), $this->pool->getItem($keys[1])->get());
    }
}
