...
class RedisCluster implements ClusterInterface, \IteratorAggregate, \Countable
{
    ...
    {
        if ($first < 0x0 || $first > 0x3fff || $last < 0x0 || $last > 0x3fff || $last < $first) {
    ...
    protected function createConnection($connectionID)
    {
        ...
        return $this->connections->create(array('host' => substr($connectionID, 0, $separator), 'port' => substr($connectionID, $separator + 1)));
    }
    /**
     * {@inheritdoc}
     */
