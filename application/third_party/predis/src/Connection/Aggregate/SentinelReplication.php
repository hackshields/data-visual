...
class SentinelReplication implements ReplicationInterface
{
     ...
     */
    public function __construct($service, array $sentinels, ConnectionFactoryInterface $connectionFactory, ReplicationStrategy $strategy = null)
    {
    ...
    {
        $this->sentinelTimeout = (double) $timeout;
    }
    /**
    ...
    public function updateSentinels()
    {
        ...
        try {
            ...
            foreach ($payload as $sentinel) {
                $this->sentinels[] = array('host' => $sentinel[3], 'port' => $sentinel[5]);
            }
    ...
    protected function querySentinelForMaster(NodeConnectionInterface $sentinel, $service)
    {
        ...
        return array('host' => $payload[0], 'port' => $payload[1], 'alias' => 'master');
    }
    /**
    ...
    protected function querySentinelForSlaves(NodeConnectionInterface $sentinel, $service)
    {
        ...
        foreach ($payload as $slave) {
            ...
            $slaves[] = array('host' => $slave[3], 'port' => $slave[5], 'alias' => "slave-{$slave[1]}");
        }
        return $slaves;
    }
    /**
    ...
    {
        return array('master', 'slaves', 'service', 'sentinels', 'connectionFactory', 'strategy');
    }
}
