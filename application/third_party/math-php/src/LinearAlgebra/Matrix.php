...
class Matrix implements \ArrayAccess, \JsonSerializable
{
    ...
    public function sampleMean() : Vector
    {
        $m = $this->m;
        $n = $this->n;
        $M = array_reduce($this->asVectors(), function (Vector $carryV, Vector $V) {
            return $carryV->add($V);
    ...
    public function luDecomposition() : array
    {
        ...
        $this->P = $P;
        return ['L' => $this->L, 'U' => $this->U, 'P' => $this->P];
    }
    /**
    ...
    public function choleskyDecomposition() : Matrix
    {
        ...
        for ($j = 0; $j < $m; $j++) {
            for ($i = 0; $i < $j + 1; $i++) {
                ...
                $L[$j][$i] = $j === $i ? sqrt($this->A[$j][$j] - $∑lⱼₓlᵢₓ) : 1 / $L[$i][$i] * ($this->A[$j][$i] - $∑lⱼₓlᵢₓ);
            }
        }
    ...
    public function croutDecomposition() : array
    {
        ...
        return ['L' => MatrixFactory::create($L), 'U' => MatrixFactory::create($U)];
    }
    /**************************************************************************
