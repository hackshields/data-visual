...
trait ValidatorTrait
{
    ...
    protected function assertUriObject($uri)
    {
        if (!$uri instanceof Uri && !$uri instanceof UriInterface) {
            throw new InvalidArgumentException(sprintf('URI passed must implement PSR-7 or League\\Uri Uri interface; received "%s"', is_object($uri) ? get_class($uri) : gettype($uri)));
        }
    }
    /**
     * validate a string
    ...
    protected static function validateString($str)
    {
        ...
        throw new InvalidArgumentException(sprintf('Expected data to be a string; received "%s"', is_object($str) ? get_class($str) : gettype($str)));
    }
    /**
    ...
    protected function validatePort($port)
    {
        ...
        $res = filter_var($port, FILTER_VALIDATE_INT, ['options' => ['min_range' => Port::MINIMUM, 'max_range' => Port::MAXIMUM]]);
        if (false === $res) {
            throw new InvalidArgumentException('The submitted port is invalid');
        }
        return $res;
