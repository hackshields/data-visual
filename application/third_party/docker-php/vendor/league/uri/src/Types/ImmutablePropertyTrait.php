...
trait ImmutablePropertyTrait
{
    ...
    {
        if (null === $value) {
            throw new InvalidArgumentException(sprintf('Expected data to be a string; received "%s"', is_object($value) ? get_class($value) : gettype($value)));
        }
        if ('' === $value) {
            return null;
        }
     ...
     */
    protected abstract function assertValidObject();
    /**
     * @inheritdoc
     */
