...
class Data extends AbstractUri implements Uri
{
     ...
     */
    public function __construct(SchemeInterface $scheme, UserInfoInterface $userInfo, HostInterface $host, PortInterface $port, PathInterface $path, QueryInterface $query, FragmentInterface $fragment)
    {
        $this->scheme = $scheme;
    ...
    public static function createFromComponents(array $components)
    {
        ...
        return new static(new Scheme($components['scheme']), new UserInfo(new User($components['user']), new Pass($components['pass'])), new Host($components['host']), new Port($components['port']), new Path($components['path']), new Query($components['query']), new Fragment($components['fragment']));
    }
    /**
    ...
    {
        return new static(new Scheme('data'), new UserInfo(), new Host(), new Port(), Path::createFromPath($path), new Query(), new Fragment());
    }
    /**
     * @inheritdoc
     */
    ...
    {
        return new static($properties['scheme'], $properties['userInfo'], $properties['host'], $properties['port'], $properties['path'], $properties['query'], $properties['fragment']);
    }
}
