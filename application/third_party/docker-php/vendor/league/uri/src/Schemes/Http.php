...
class Http extends AbstractHierarchicalUri implements UriInterface
{
     ...
     * @inheritdoc
     */
    protected static $supportedSchemes = ['http' => 80, 'https' => 443];
    /**
     * @inheritdoc
     */
    protected function isValid()
    {
        return $this->isValidGenericUri() && $this->isValidHierarchicalUri();
    }
    /**
     * @inheritdoc
     */
    ...
    {
        return static::createFromString(static::fetchServerScheme($server) . '://' . static::fetchServerUserInfo($server) . static::fetchServerHost($server) . static::fetchServerPort($server) . static::fetchServerRequestUri($server));
    }
    /**
