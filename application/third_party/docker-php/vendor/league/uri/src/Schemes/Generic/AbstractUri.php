...
abstract class AbstractUri
{
    ...
    public function withUserInfo($user, $password = null)
    {
        ...
        if ($this->userInfo->getUser() === $userInfo->getUser() && $this->userInfo->getPass() === $userInfo->getPass()) {
            return $this;
        }
        $clone = clone $this;
    ...
    protected function getSchemeSpecificPart()
    {
        ...
        $res = array_filter([$this->userInfo->getContent(), $this->host->getContent(), $this->port->getContent()], function ($value) {
            return null !== $value;
        });
        if (!empty($res)) {
            $authority = '//' . $authority;
        }
        return $authority . $this->path->getUriComponent() . $this->query->getUriComponent() . $this->fragment->getUriComponent();
    }
    /**
     * @inheritdoc
     */
    ...
    {
        if (!$this->isValid()) {
            throw new InvalidArgumentException(sprintf('The URI components will produce a `%s` instance in invalid state', get_class($this)));
        }
    }
    /**
    ...
    protected function assertSupportedScheme()
    {
        if (!array_key_exists($this->scheme->__toString(), static::$supportedSchemes)) {
            throw new InvalidArgumentException(sprintf('The submitted scheme is unsupported by `%s`', get_class($this)));
        }
    }
}
