...
function uri_reference($uri, $base_uri = null)
{
    if (!$uri instanceof Uri && !$uri instanceof UriInterface) {
        throw new InvalidArgumentException('URI passed must implement PSR-7 UriInterface or League\\Uri Uri interface');
    }
    if (null !== $base_uri && (!$base_uri instanceof Uri && !$base_uri instanceof UriInterface)) {
        throw new InvalidArgumentException('The base URI passed must implement PSR-7 UriInterface or League\\Uri Uri interface');
    }
    $infos = ['absolute_uri' => false, 'network_path' => false, 'absolute_path' => false, 'relative_path' => false, 'same_document' => false];
    static $normalizer;
    if (null === $normalizer) {
