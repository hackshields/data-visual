...
class Resolve extends AbstractUriModifier
{
    ...
    public function __invoke($target)
    {
        ...
        if ($meta['absolute_uri']) {
            return $target->withPath((new Path($target_path))->withoutDotSegments()->__toString());
        }
        if ($meta['network_path']) {
            return $target->withScheme($this->uri->getScheme())->withPath((new Path($target_path))->withoutDotSegments()->__toString());
        }
        $user_info = explode(':', $this->uri->getUserInfo(), 2);
        $components = $this->resolvePathAndQuery($target_path, $target->getQuery());
        return $target->withPath($this->formatPath($components['path']))->withQuery($components['query'])->withHost($this->uri->getHost())->withPort($this->uri->getPort())->withUserInfo((string) array_shift($user_info), array_shift($user_info))->withScheme($this->uri->getScheme());
    }
    /**
