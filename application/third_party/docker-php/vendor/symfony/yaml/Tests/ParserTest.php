...
# another comment
EOT;
        $expected = array('a' => array('b' => array(), 'd' => 1.1));
        $this->assertSame($expected, $this->parser->parse($yaml));
    }
...
test:
  ...
  yes you can
EOT;
        $expected = array('test' => 'You can have things that don\'t look like strings here true yes you can');
        $this->assertSame($expected, $this->parser->parse($yaml));
        $yaml = <<<'EOT'
a:
    b
...
'~': 'null'
EOF;
        $expected = array('1.2' => 'bar', '1.3' => 'baz', 'true' => 'foo', 'false' => 'bar', 'null' => 'null', '~' => 'null');
...
foo:
    ...
    bar: baz
YAML;
        $expected = array('foo' => array('foobar' => 'foo #bar', 'bar' => 'baz'));
        $this->assertSame($expected, $this->parser->parse($yaml));
    }
...
EOF;
    ...
    {
        return array('scalars' => array(array('foo' => new TaggedValue('inline', 'bar'), 'quz' => new TaggedValue('long', 'this is a long text')), <<<YAML
foo: !inline bar
quz: !long >
  this is a long
  text
YAML
), 'sequences' => array(array(new TaggedValue('foo', array('yaml')), new TaggedValue('quz', array('bar'))), <<<YAML
- !foo
    - yaml
- !quz [bar]
YAML
...
YAML
), 'spaces-around-tag-value-in-sequence' => array(array(new TaggedValue('foo', 'bar')), '[ !foo bar ]'));
    }
...
mergekeyderef:
    ...
    <<: *quux
YAML;
        $expected = (object) array('mergekeyrefdef' => (object) array('a' => 'foo', 'b' => 'bar', 'c' => 'baz'), 'mergekeyderef' => (object) array('d' => 'quux', 'b' => 'bar', 'c' => 'baz'));
        $this->assertEquals($expected, $this->parser->parse($yaml, Yaml::PARSE_OBJECT_FOR_MAP));
    }
    /**
     * @expectedException \Symfony\Component\Yaml\Exception\ParseException
...
foo:
  - bar: "foobar"
    ...
    baz: "foobaz"
YAML;
        $expected = array('foo' => array(array('bar' => 'foobar', 'baz' => 'foobaz')));
...
foo:
    - bar:
        ...
        baz: [1, 2, 3]
YAML;
        $expected = array('foo' => array(array('bar' => array('baz' => array(1, 2, 3)))));
...
foo:
  -
    bar: foobar
YAML;
        $expected = array('foo' => array(array('bar' => 'foobar')));
