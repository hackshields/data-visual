...
class SerializerTest extends TestCase
{
    ...
    public function testDeserializeAndSerializeAbstractObjectsWithTheClassMetadataDiscriminatorResolver()
    {
        ...
        $loaderMock->method('hasMetadataFor')->will($this->returnValueMap(array(array(AbstractDummy::class, true))));
        $loaderMock->method('getMetadataFor')->will($this->returnValueMap(array(array(AbstractDummy::class, new ClassMetadata(AbstractDummy::class, new ClassDiscriminatorMapping('type', array('first' => AbstractDummyFirstChild::class, 'second' => AbstractDummySecondChild::class)))))));
    ...
    public function testDeserializeAndSerializeInterfacedObjectsWithTheClassMetadataDiscriminatorResolverAndGroups()
    {
        ...
        $example->two = 2;
        $serializer = $this->serializerWithClassDiscriminator();
        $deserialized = $serializer->deserialize('{"type":"one","one":1,"two":2}', DummyMessageInterface::class, 'json', array('groups' => array('two')));
        $this->assertEquals($example, $deserialized);
        $serialized = $serializer->serialize($deserialized, 'json', array('groups' => array('two')));
