...
class AbstractObjectNormalizerTest extends TestCase
{
    ...
    public function testDenormalizeWithExtraAttributesAndNoGroupsWithMetadataFactory()
    {
        ...
        $normalizer->denormalize(array('fooFoo' => 'foo', 'fooBar' => 'bar', 'bar' => 'bar'), Dummy::class, 'any', array('allow_extra_attributes' => false));
    }
    public function testDenormalizeCollectionDecodedFromXmlWithOneChild()
    {
        $denormalizer = $this->getDenormalizerForDummyCollection();
        $dummyCollection = $denormalizer->denormalize(array('children' => array('bar' => 'first')), DummyCollection::class, 'xml');
    ...
    {
        $denormalizer = $this->getDenormalizerForDummyCollection();
        $dummyCollection = $denormalizer->denormalize(array('children' => array(array('bar' => 'first'), array('bar' => 'second'))), DummyCollection::class, 'xml');
        $this->assertInstanceOf(DummyCollection::class, $dummyCollection);
        $this->assertInternalType('array', $dummyCollection->children);
    ...
    private function getDenormalizerForDummyCollection()
    {
        ...
        $extractor->method('getTypes')->will($this->onConsecutiveCalls(array(new Type('array', false, null, true, new Type('int'), new Type('object', false, DummyChild::class))), null));
    ...
    public function testExtraAttributesException()
    {
        ...
        $normalizer->denormalize(array(), \stdClass::class, 'xml', array('allow_extra_attributes' => false));
    }
}
