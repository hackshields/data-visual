...
class ObjectNormalizerTest extends TestCase
{
    ...
    {
        $obj = $this->normalizer->denormalize(array('foo' => 'foo', 'bar' => 'bar', 'baz' => true, 'fooBar' => 'foobar'), __NAMESPACE__ . '\\ObjectDummy', 'any');
    ...
    {
        $obj = $this->normalizer->denormalize(array('foo' => 'foo', 'bar' => 'bar', 'baz' => true, 'fooBar' => 'foobar'), __NAMESPACE__ . '\\ObjectConstructorDummy', 'any');
        $this->assertEquals('foo', $obj->getFoo());
        $this->assertEquals('bar', $obj->bar);
        $this->assertTrue($obj->isBaz());
    }
    ...
    {
        $obj = $this->normalizer->denormalize(array('foo' => 'test', 'baz' => array(1, 2, 3)), __NAMESPACE__ . '\\ObjectConstructorOptionalArgsDummy', 'any');
    ...
    {
        $obj = $this->normalizer->denormalize(array('bar' => 'test'), __NAMESPACE__ . '\\ObjectConstructorArgsWithDefaultValueDummy', 'any');
    ...
    {
        $data = array('id' => 10, 'unknown' => array('foo' => 'oof', 'bar' => 'rab'));
        $normalizer = new ObjectNormalizer();
        $serializer = new Serializer(array($normalizer));
        $normalizer->setSerializer($serializer);
    ...
    public function testFillWithEmptyDataWhenMissingData()
    {
        $data = array('foo' => 10);
        $normalizer = new ObjectNormalizer();
        $result = $normalizer->denormalize($data, DummyValueObject::class, 'json', array('default_constructor_arguments' => array(DummyValueObject::class => array('foo' => '', 'bar' => ''))));
    ...
    public function provideCallbacks()
    {
        return array(array(array('bar' => function ($bar) {
            ...
            return 'baz';
        }), 'baz', array('foo' => '', 'bar' => 'baz', 'baz' => true), 'Change a string'), array(array('bar' => function ($value, $object, $attributeName, $format, $context) {
    ...
    public function testMaxDepth()
    {
        ...
        $expected = array('bar' => null, 'foo' => 'level1', 'child' => array('bar' => null, 'foo' => 'level2', 'child' => array('bar' => null, 'child' => null, 'foo' => 'handler')));
    ...
    public function testDenomalizeRecursive()
    {
        ...
        $obj = $serializer->denormalize(array('inner' => array('foo' => 'foo', 'bar' => 'bar'), 'date' => '1988/01/21', 'inners' => array(array('foo' => 1), array('foo' => 2))), ObjectOuter::class);
    ...
    public function testAttributesContextNormalize()
    {
        ...
        $this->assertEquals(array('foo' => 'foo', 'baz' => true, 'object' => array('foo' => 'innerFoo')), $serializer->normalize($objectDummy, null, $context));
        $context = array('attributes' => array('foo', 'baz', 'object'));
        $this->assertEquals(array('foo' => 'foo', 'baz' => true, 'object' => array('foo' => 'innerFoo', 'bar' => 'innerBar')), $serializer->normalize($objectDummy, null, $context));
    }
    public function testAttributesContextDenormalize()
    {
        ...
        $this->assertEquals($objectOuter, $serializer->denormalize(array('foo' => 'foo', 'bar' => 'bar', 'date' => '2017-02-03', 'inner' => array('foo' => 'innerFoo', 'bar' => 'innerBar')), ObjectOuter::class, null, $context));
    }
    public function testAttributesContextDenormalizeConstructor()
    {
        ...
        $this->assertEquals($obj, $serializer->denormalize(array('foo' => 'b', 'inner' => array('foo' => 'foo', 'bar' => 'bar')), DummyWithConstructorObjectAndDefaultValue::class, null, $context));
    }
    public function testNormalizeSameObjectWithDifferentAttributes()
    {
        ...
        $this->assertEquals(array('foo' => array('bar' => 'foo.bar'), 'bar' => array('foo' => 'bar.foo')), $this->normalizer->normalize($dummy, 'json', array('attributes' => array('foo' => array('bar'), 'bar' => array('foo')))));
    }
}
class ObjectDummy
{
