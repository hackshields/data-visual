...
class PropertyNormalizerTest extends TestCase
{
    ...
    public function testNormalize()
    {
        ...
        $this->assertEquals(array('foo' => 'foo', 'bar' => 'bar', 'camelCase' => 'camelcase'), $this->normalizer->normalize($obj, 'any'));
    }
    public function testDenormalize()
    {
        $obj = $this->normalizer->denormalize(array('foo' => 'foo', 'bar' => 'bar'), __NAMESPACE__ . '\\PropertyDummy', 'any');
    ...
    public function testNormalizeWithParentClass()
    {
        ...
        $this->assertEquals(array('foo' => 'foo', 'bar' => 'bar', 'kevin' => 'Kevin', 'coopTilleuls' => 'coop', 'fooBar' => null, 'symfony' => null, 'baz' => 'baz'), $this->normalizer->normalize($group, 'any'));
    }
    public function testDenormalizeWithParentClass()
    {
        $obj = $this->normalizer->denormalize(array('foo' => 'foo', 'bar' => 'bar', 'kevin' => 'Kevin', 'baz' => 'baz'), GroupDummyChild::class, 'any');
    ...
    {
        $obj = $this->normalizer->denormalize(array('foo' => null, 'bar' => 'bar'), __NAMESPACE__ . '\\PropertyConstructorDummy', '
            any');
    ...
    public function testGroupsNormalize()
    {
        ...
        $this->assertEquals(array('bar' => 'bar'), $this->normalizer->normalize($obj, null, array(PropertyNormalizer::GROUPS => array('c'))));
        // The PropertyNormalizer is also able to hydrate properties from parent classes
        $this->assertEquals(array('symfony' => 'symfony', 'foo' => 'foo', 'fooBar' => 'fooBar', 'bar' => 'bar', 'kevin' => 'kevin', 'coopTilleuls' => 'coopTilleuls'), $this->normalizer->normalize($obj, null, array(PropertyNormalizer::GROUPS => array('a', 'c'))));
    }
    public function testGroupsDenormalize()
    {
        ...
        $normalized = $this->normalizer->denormalize($toNormalize, 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, array(PropertyNormalizer::GROUPS => array('a')));
        $this->assertEquals($obj, $normalized);
        $obj->setBar('bar');
        $normalized = $this->normalizer->denormalize($toNormalize, 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, array(PropertyNormalizer::GROUPS => array('a', 'b')));
        $this->assertEquals($obj, $normalized);
    }
    public function testGroupsNormalizeWithNameConverter()
    {
        ...
        $this->assertEquals(array('bar' => null, 'foo_bar' => '@dunglas', 'symfony' => '@coopTilleuls'), $this->normalizer->normalize($obj, null, array(PropertyNormalizer::GROUPS => array('name_converter'))));
    }
    public function testGroupsDenormalizeWithNameConverter()
    {
        ...
        $obj->setFooBar('@dunglas');
        $obj->setSymfony('@coopTilleuls');
        $this->assertEquals($obj, $this->normalizer->denormalize(array('bar' => null, 'foo_bar' => '@dunglas', 'symfony' => '@coopTilleuls', 'coop_tilleuls' => 'les-tilleuls.coop'), 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, array(PropertyNormalizer::GROUPS => array('name_converter'))));
    }
    public function provideCallbacks()
    {
        return array(array(array('bar' => function ($bar) {
            return 'baz';
        }), 'baz', array('foo' => '', 'bar' => 'baz'), 'Change a string'), array(array('bar' => function ($bar) {
        }), 'baz', array('foo' => '', 'bar' => null), 'Null an item'), array(array('bar' => function ($bar) {
            return $bar->format('d-m-Y H:i:s');
        }), new \DateTime('2011-09-10 06:30:00'), array('foo' => '', 'bar' => '10-09-2011 06:30:00'), 'Format a date'), array(array('bar' => function ($bars) {
            $foos = '';
            foreach ($bars as $bar) {
                $foos .= $bar->getFoo();
            }
            return $foos;
        }), array(new PropertyConstructorDummy('baz', ''), new PropertyConstructorDummy('quux', '')), array('foo' => '', 'bar' => 'bazquux'), 'Collect a property'), array(array('bar' => function ($bars) {
            return \count($bars);
        }), array(new PropertyConstructorDummy('baz', ''), new PropertyConstructorDummy('quux', '')), array('foo' => '', 'bar' => 2), 'Count a property'));
    }
    /**
