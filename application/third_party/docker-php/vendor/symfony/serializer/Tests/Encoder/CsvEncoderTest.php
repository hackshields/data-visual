...
, $this->encoder->encode($value, 'csv'));
    ...
    {
        $value = array(array('foo' => 'hello', 'bar' => 'hey ho'), array('foo' => 'hi', 'bar' => 'let\'s go'));
        $this->assertEquals(<<<'CSV'
foo,bar
hello,"hey ho"
hi,"let's go"
...
, $this->encoder->encode('foo', 'csv'));
    ...
    {
        $value = array('foo' => 'hello', 'bar' => array(array('id' => 'yo', 1 => 'wesh'), array('baz' => 'Halo', 'foo' => 'olá')));
        $this->assertEquals(<<<'CSV'
...
CSV
, $this->encoder->encode($value, 'csv', array(CsvEncoder::DELIMITER_KEY => ';', CsvEncoder::ENCLOSURE_KEY => "'", CsvEncoder::ESCAPE_CHAR_KEY => '|', CsvEncoder::KEY_SEPARATOR_KEY => '-')));
    }
    ...
    {
        $value = array(array('a' => array('foo', 'bar')), array('a' => array(), 'b' => 'baz'), array('a' => array('bar', 'foo'), 'c' => 'pong'));
        $csv = <<<CSV
a.0,a.1,c,b
foo,bar,,
,,,baz
...
CSV;
    ...
    {
        $context = array(CsvEncoder::HEADERS_KEY => array('b', 'c'));
        $value = array(array('a' => 'foo', 'b' => 'bar'));
        $csv = <<<CSV
b,c,a
bar,,foo
...
CSV
, $this->encoder->encode(array('=2+3'), 'csv', array(CsvEncoder::ESCAPE_FORMULAS_KEY => true)));
        $this->assertSame(<<<'CSV'
0
"	-2+3"
...
CSV
, $this->encoder->encode(array('@MyDataColumn'), 'csv', array(CsvEncoder::ESCAPE_FORMULAS_KEY => true)));
    }
...
, 'csv'));
    ...
    {
        $expected = array(array('foo' => 'a', 'bar' => 'b'), array('foo' => 'c', 'bar' => 'd'), array('foo' => 'f'));
        $this->assertEquals($expected, $this->encoder->decode(<<<'CSV'
foo,bar
a,b
c,d
...
, 'csv', array(CsvEncoder::AS_COLLECTION_KEY => true)));
    ...
    {
        $expected = array(array('foo' => 'bar', 'relations' => array(array('a' => 'b'), array('a' => 'b'))), array('foo' => 'bat', 'relations' => array(array('a' => 'b'), array('a' => ''))), array('foo' => 'bat', 'relations' => array(array('a' => 'b'))), array('foo' => 'baz', 'relations' => array(array('a' => 'c'), array('a' => 'c'))));
        $this->assertEquals($expected, $this->encoder->decode(<<<'CSV'
...
, 'csv'));
    ...
    {
        $expected = array(array('foo' => 'a', 'bar' => array('baz' => array('bat' => 'b'))), array('foo' => 'c', 'bar' => array('baz' => array('bat' => 'd'))));
        $this->assertEquals($expected, $this->encoder->decode(<<<'CSV'
foo,bar.baz.bat
a,b
c,d
