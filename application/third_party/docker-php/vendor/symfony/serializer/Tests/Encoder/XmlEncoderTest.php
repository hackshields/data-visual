...
class XmlEncoderTest extends TestCase
{
    ...
    {
        $obj = new ScalarDummy();
        $obj->xmlFoo = array('foo-bar' => array('@id' => 1, '@name' => 'Bar'), 'Foo' => array('Bar' => 'Test', '@Type' => 'test'), 'föo_bär' => 'a', 'Bar' => array(1, 2, 3), 'a' => 'b');
    ...
    {
        $obj = new ScalarDummy();
        $obj->xmlFoo = array('foo-bar' => 'a', 'foo_bar' => 'a', 'föo_bär' => 'a');
    ...
    public function testEncodeXmlAttributes()
    {
        ...
        $context = array('xml_version' => '1.1', 'xml_encoding' => 'utf-8', 'xml_standalone' => true);
...
XML;
        $context = array('xml_format_output' => true);
        $this->assertSame($expected, $this->encoder->encode($array, 'xml', $context));
    }
    ...
    {
        $array = array('firstname' => 'Paul <or Me>');
...
XML;
        $this->assertSame(array('@index' => 1.8207724176001167E+59, '#' => 'Name'), $this->encoder->decode($source, 'xml'));
    }
...
XML;
        ...
        $expected = array('@a' => '018', '@b' => '-12.11', 'node' => array('@a' => '018', '@b' => '-12.11', '#' => ''));
...
XML;
    ...
    {
        $expected = array('firstname' => 'Paul <or Me>');
        $xml = '<?xml version="1.0"?>' . "\n" . '<response><firstname><![CDATA[Paul <or Me>]]></firstname></response>' . "\n";
    ...
    public function testDecodeRootAttributes()
    {
        ...
        $expected = array('firstname' => 'Peter', 'lastname' => 'Mac Calloway', '@gender' => 'M');
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
    public function testDecodeArray()
    {
        ...
        $expected = array('people' => array('person' => array(array('firstname' => 'Benjamin', 'lastname' => 'Alexandre'), array('firstname' => 'Damien', 'lastname' => 'Clay'))));
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
...
XML;
        $expected = array('person' => array(array('firstname' => 'Benjamin', 'lastname' => 'Alexandre'), array('firstname' => 'Damien', 'lastname' => 'Clay')));
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
...
XML;
        ...
        $serializer = new Serializer(array(new CustomNormalizer()), array('xml' => new XmlEncoder()));
        $this->encoder->setSerializer($serializer);
        $expected = array('person' => array(array('firstname' => 'Benjamin', 'lastname' => 'Alexandre', '#comment' => ' This comment should be decoded. '), array('firstname' => 'Damien', 'lastname' => 'Clay')));
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
...
XML;
    ...
    {
        $obj = new ScalarDummy();
        $obj->xmlFoo = array('foo-bar' => array('@key' => 'value', 'item' => array('@key' => 'key', 'key-val' => 'val')), 'Foo' => array('Bar' => 'Test', '@Type' => 'test'), 'föo_bär' => 'a', 'Bar' => array(1, 2, 3), 'a' => 'b');
        $expected = array('foo-bar' => array('@key' => 'value', 'key' => array('@key' => 'key', 'key-val' => 'val')), 'Foo' => array('Bar' => 'Test', '@Type' => 'test'), 'föo_bär' => 'a', 'Bar' => array(1, 2, 3), 'a' => 'b');
    ...
    {
        return array('@xmlns' => 'http://www.w3.org/2005/Atom', '@xmlns:app' => 'http://www.w3.org/2007/app', '@xmlns:media' => 'http://search.yahoo.com/mrss/', '@xmlns:gd' => 'http://schemas.google.com/g/2005', '@xmlns:yt' => 'http://gdata.youtube.com/schemas/2007', 'qux' => '1', 'app:foo' => 'foo', 'yt:bar' => array('a', 'b'), 'media:baz' => array('media:key' => 'val', 'media:key2' => 'val', 'A B' => 'bar', 'item' => array(array('title' => 'title1'), array('title' => 'title2')), 'Barry' => array('@size' => 'large', 'FooBar' => array('Baz' => 'Ed', '@gd:id' => 1))));
    }
...
XML;
    ...
    private function createMockDateTimeNormalizer()
    {
        ...
        $mock->expects($this->once())->method('normalize')->with(new \DateTime($this->exampleDateTimeString), 'xml', array())->willReturn($this->exampleDateTimeString);
