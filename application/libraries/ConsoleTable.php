...
class ConsoleTable
{
     ...
     * @var array
     */
    public $_cell_lengths = array();
    /**
     * Heights of the rows.
     *
     * @var array
     */
    public $_row_heights = array();
    /**
     ...
     * @var array
     */
    public $_calculateTotals = NULL;
    /**
     * Alignment of the columns.
     *
     * @var array
     */
    public $_col_align = array();
    /**
     * Default alignment of columns.
     *
     * @var integer
     */
    public $_defaultAlign = NULL;
    /**
     ...
     * @var array
     */
    public $_border = array("intersection" => "+", "horizontal" => "-", "vertical" => "|");
    /**
     ...
     * @var array
     */
    public $_borderVisibility = array("top" => true, "right" => true, "bottom" => true, "left" => true, "inner" => true);
    /**
     * Whether the data has ANSI colors.
     *
     * @var Console_Color2
     */
    public $_ansiColor = false;
    /**
     * Constructor.
     *
     ...
     */
    public function __construct($align = CONSOLE_TABLE_ALIGN_LEFT, $border = CONSOLE_TABLE_BORDER_ASCII, $padding = 1, $charset = NULL, $color = false)
    {
    ...
    public function setBorder($border)
    {
        ...
        $this->_border = array("intersection" => $intersection, "horizontal" => $horizontal, "vertical" => $vertical);
    }
    /**
    ...
    {
        $this->_borderVisibility = array_merge($this->_borderVisibility, array_intersect_key($visibility, $this->_borderVisibility));
    }
    /**
        ...
        for ($i = 0; $i < count($this->_data); $i++) {
            for ($j = 0; $j < count($this->_data[$i]); $j++) {
                if ($this->_data[$i] !== CONSOLE_TABLE_HORIZONTAL_RULE && $this->_strlen($this->_data[$i][$j]) < $this->_cell_lengths[$j]) {
                    $this->_data[$i][$j] = $this->_strpad($this->_data[$i][$j], $this->_cell_lengths[$j], " ", $this->_col_align[$j]);
                }
            }
            if ($this->_data[$i] !== CONSOLE_TABLE_HORIZONTAL_RULE) {
    ...
    public function _getSeparator()
    {
        ...
        $implode_char = str_repeat($horizontal, $this->_padding) . $intersection . str_repeat($horizontal, $this->_padding);
        ...
        for ($j = 0; $j < count($this->_headers); $j++) {
            for ($i = 0; $i < count($this->_headers[$j]); $i++) {
                if ($this->_strlen($this->_headers[$j][$i]) < $this->_cell_lengths[$i]) {
                    $this->_headers[$j][$i] = $this->_strpad($this->_headers[$j][$i], $this->_cell_lengths[$i], " ", $this->_col_align[$i]);
                }
            }
        }
        $vertical = $this->_border["vertical"];
    ...
    public function _strpad($input, $length, $pad = " ", $type = STR_PAD_RIGHT)
    {
        ...
        switch ($type) {
            ...
            case STR_PAD_BOTH:
                ...
                $output = $this->_substr(str_repeat($pad, ceil($left / $pad_length)), 0, $left, $this->_charset) . $input . $this->_substr(str_repeat($pad, ceil($right / $pad_length)), 0, $right, $this->_charset);
                break;
            case STR_PAD_RIGHT:
