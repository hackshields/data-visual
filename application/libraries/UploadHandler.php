...
class UploadHandler
{
    ...
    {
        $this->response = array();
        $this->options = array("script_url" => $this->get_full_url() . "/" . $this->basename($this->get_server_var("SCRIPT_NAME")), "upload_dir" => dirname($this->get_server_var("SCRIPT_FILENAME")) . "/files/", "upload_url" => $this->get_full_url() . "/files/", "input_stream" => "php://input", "user_dirs" => false, "mkdir_mode" => 493, "param_name" => "files", "delete_type" => "DELETE", "access_control_allow_origin" => "*", "access_control_allow_credentials" => false, "access_control_allow_methods" => array("OPTIONS", "HEAD", "GET", "POST", "PUT", "PATCH", "DELETE"), "access_control_allow_headers" => array("Content-Type", "Content-Range", "Content-Disposition"), "redirect_allow_target" => "/^" . preg_quote(parse_url($this->get_server_var("HTTP_REFERER"), PHP_URL_SCHEME) . "://" . parse_url($this->get_server_var("HTTP_REFERER"), PHP_URL_HOST) . "/", "/") . "/", "download_via_php" => false, "readfile_chunk_size" => 10 * 1024 * 1024, "inline_file_types" => "/\\.(gif|jpe?g|png)\$/i", "accept_file_types" => "/.+\$/i", "max_file_size" => NULL, "min_file_size" => 1, "max_number_of_files" => NULL, "image_file_types" => "/\\.(gif|jpe?g|png)\$/i", "correct_image_extensions" => false, "max_width" => NULL, "max_height" => NULL, "min_width" => 1, "min_height" => 1, "discard_aborted_uploads" => true, "image_library" => 1, "convert_bin" => "convert", "identify_bin" => "identify", "image_versions" => array("" => array("auto_orient" => true), "thumbnail" => array("max_width" => 80, "max_height" => 80)), "print_response" => true);
        if ($options) {
    ...
    protected function validate($uploaded_file, $file, $error, $index)
    {
        ...
        if ($post_max_size && $post_max_size < $content_length) {
        ...
        if (is_int($this->options["max_number_of_files"]) && $this->options["max_number_of_files"] <= $this->count_file_objects() && !is_file($this->get_upload_path($file->name))) {
        ...
        if (($max_width || $max_height || $min_width || $min_height) && preg_match($this->options["image_file_types"], $file->name)) {
            ...
            if ($this->options["image_versions"][""]["auto_orient"] && function_exists("exif_read_data") && ($exif = @exif_read_data($uploaded_file)) && 5 <= (int) $exif["Orientation"]) {
                $tmp = $img_width;
        ...
        if (!empty($img_width)) {
            if ($max_width && $max_width < $img_width) {
                $file->error = $this->get_error_message("max_width");
                return false;
            }
            if ($max_height && $max_height < $img_height) {
    ...
    protected function gd_imageflip($image, $mode)
    {
        ...
        switch ($mode) {
            case "1":
                $src_y = $new_height - 1;
                $src_height = 0 - $new_height;
                break;
            case "2":
                $src_x = $new_width - 1;
                $src_width = 0 - $new_width;
                break;
            case "3":
                $src_y = $new_height - 1;
                $src_height = 0 - $new_height;
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = NULL, $content_range = NULL)
    {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $append_file = $content_range && is_file($file_path) && $this->get_file_size($file_path) < $file->size;
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                if ($append_file) {
