...
function smarty_function_html_select_time($params, Smarty_Internal_Template $template)
{
    ...
    if (isset($params["time"]) && is_array($params["time"])) {
        if (isset($params["time"][$prefix . "Hour"])) {
            ...
            if (isset($params["time"][$field_array][$prefix . "Hour"])) {
                foreach (array("H" => "Hour", "i" => "Minute", "s" => "Second") as $_elementKey => $_elementName) {
                    $_variableName = "_" . strtolower($_elementName);
    ...
    if ($display_meridian && !$use_24_hours) {
        ...
        $_html_meridian .= "<option value=\"am\"" . (0 < $_hour && $_hour < 12 ? " selected=\"selected\"" : "") . ">AM</option>" . $option_separator . "<option value=\"pm\"" . ($_hour < 12 ? "" : " selected=\"selected\"") . ">PM</option>" . $option_separator . "</select>";
    }
    $_html = "";
