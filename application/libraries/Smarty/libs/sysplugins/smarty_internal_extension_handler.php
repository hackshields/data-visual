            ...
            if (!empty($match[2]) && !isset($smarty->ext->{$name})) {
                ...
                if (!class_exists($class)) {
                    ...
                    if (!isset($this->resolvedProperties[$match[0]][$objType])) {
                        $property = isset($this->resolvedProperties["property"][$basename]) ? $this->resolvedProperties["property"][$basename] : ($property = $this->resolvedProperties["property"][$basename]);
                        if ($property !== false) {
                            if (property_exists($data, $property)) {
                                $this->resolvedProperties[$match[0]][$objType] = 1;
                                $propertyType = $this->resolvedProperties[$match[0]][$objType];
                            } else {
                                if (property_exists($smarty, $property)) {
                                    $this->resolvedProperties[$match[0]][$objType] = 2;
                                    $propertyType = $this->resolvedProperties[$match[0]][$objType];
                                } else {
