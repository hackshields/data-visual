...
class Smarty_Internal_Template extends Smarty_Internal_TemplateBase
{
    ...
    public function _subTemplateRender($template, $cache_id, $compile_id, $caching, $cache_lifetime, $data, $scope, $forceTplCache, $uid = NULL, $content_func = NULL)
    {
        ...
        $tpl->scope = $scope;
        if (!isset(self::$tplObjCache[$tpl->templateId]) && !$tpl->source->handler->recompiled && ($forceTplCache || isset(self::$subTplInfo[$tpl->template_resource]) && 1 < self::$subTplInfo[$tpl->template_resource] || $tpl->_isSubTpl() && isset(self::$tplObjCache[$tpl->parent->templateId]))) {
    ...
    public function _assignInScope($varName, $value, $nocache = false, $scope = 0)
    {
        ...
        if (0 <= $scope && (0 < $scope || 0 < $this->scope)) {
    ...
    public function _decodeProperties(Smarty_Internal_Template $tpl, $properties, $cache = false)
    {
        ...
        if (!empty($properties["file_dependency"]) && (!$cache && $tpl->compile_check || $tpl->compile_check === Smarty::COMPILECHECK_ON)) {
            foreach ($properties["file_dependency"] as $_file_to_check) {
                ...
                if ($mtime === false || $_file_to_check[1] < $mtime) {
                    $is_valid = false;
                    break;
                }
            }
        ...
        if ($cache) {
            if ($tpl->caching === Smarty::CACHING_LIFETIME_SAVED && 0 <= $properties["cache_lifetime"] && $tpl->cached->timestamp + $properties["cache_lifetime"] < time()) {
                $is_valid = false;
            }
