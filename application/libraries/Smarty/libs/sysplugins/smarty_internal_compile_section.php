...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        $incFor = array();
        $cmpFor = array();
        $propValue = array("index" => (string) $sectionVar . "->value['index']", "show" => "true", "step" => 1, "iteration" => (string) $local . "iteration");
        ...
        if (isset($namedAttr["iteration"])) {
            $propValue["iteration"] = (string) $sectionVar . "->value['iteration']";
        }
        $incFor["iteration"] = (string) $propValue["iteration"] . "++";
        $initFor["iteration"] = (string) $propValue["iteration"] . " = 1";
        if ($propType["step"] === 0) {
            if ($propValue["step"] === 1) {
                $incFor["index"] = (string) $sectionVar . "->value['index']++";
            } else {
                if (1 < $propValue["step"]) {
                    $incFor["index"] = (string) $sectionVar . "->value['index'] += " . $propValue["step"];
                } else {
                    $incFor["index"] = (string) $sectionVar . "->value['index'] -= " . (0 - $propValue["step"]);
                }
            }
        } else {
            $incFor["index"] = (string) $sectionVar . "->value['index'] += " . $propValue["step"];
        ...
        if (!isset($propValue["max"])) {
            ...
            if ($propType["max"] !== 0) {
                $propValue["max"] = (string) $propValue["max"] . " < 0 ? " . $propValue["loop"] . " : " . $propValue["max"];
                $propType["max"] = 1;
            } else {
                if ($propValue["max"] < 0) {
                    $propValue["max"] = $propValue["loop"];
        ...
        if (!isset($propValue["start"])) {
            $start_code = array(1 => (string) $propValue["step"] . " > 0 ? ", 2 => "0", 3 => " : ", 4 => $propValue["loop"], 5 => " - 1");
            if ($propType["loop"] === 0) {
                $start_code[5] = "";
            ...
            if ($propType["step"] === 0) {
                if (0 < $propValue["step"]) {
                    ...
                    $start_code[3] = "";
                    list(, , $start_code[1], $start_code[2]) = $start_code;
            ...
            $start_code = array(1 => (string) $propValue["start"] . " < 0 ? ", 2 => "max(", 3 => (string) $propValue["step"] . " > 0 ? ", 4 => "0", 5 => " : ", 6 => "-1", 7 => ", ", 8 => (string) $propValue["start"] . " + " . $propValue["loop"], 10 => ")", 11 => " : ", 12 => "min(", 13 => $propValue["start"], 14 => ", ", 15 => (string) $propValue["step"] . " > 0 ? ", 16 => $propValue["loop"], 17 => " : ", 18 => $propType["loop"] === 0 ? $propValue["loop"] - 1 : (string) $propValue["loop"] . " - 1", 19 => ")");
            if ($propType["step"] === 0) {
                $start_code[17] = "";
                list(, , , , , $start_code[3], , , , , , , , , , $start_code[5], , $start_code[15]) = $start_code;
                if (0 < $propValue["step"]) {
                    $start_code[18] = "";
            ...
            if ($propType["start"] === 0) {
                ...
                if ($propValue["start"] < 0) {
                    ...
                    if ($propType["start"] === 0) {
                        $start_code = array(max(0 < $propValue["step"] ? 0 : -1, $propValue["start"] + $propValue["loop"]));
                    }
                } else {
                    for ($i = 1; $i <= 11; $i++) {
                        $start_code[$i] = "";
                    ...
                    if ($propType["start"] === 0) {
                        $start_code = array(min(0 < $propValue["step"] ? $propValue["loop"] : $propValue["loop"] - 1, $propValue["start"]));
                    }
                }
            }
            $propValue["start"] = join("", $start_code);
        ...
        if ($propType["start"] !== 0) {
            ...
            $propValue["start"] = (string) $local . "start";
        }
        $initFor["index"] = (string) $sectionVar . "->value['index'] = " . $propValue["start"];
        if (!isset($_attr["start"]) && !isset($_attr["step"]) && !isset($_attr["max"])) {
            ...
            if ($propType["total"] === 0) {
                $propValue["total"] = min(ceil((0 < $propValue["step"] ? $propValue["loop"] - $propValue["start"] : (int) $propValue["start"] + 1) / abs($propValue["step"])), $propValue["max"]);
            } else {
                $total_code = array(1 => "min(", 2 => "ceil(", 3 => "(", 4 => (string) $propValue["step"] . " > 0 ? ", 5 => $propValue["loop"], 6 => " - ", 7 => $propValue["start"], 8 => " : ", 9 => $propValue["start"], 10 => "+ 1", 11 => ")", 12 => "/ ", 13 => "abs(", 14 => $propValue["step"], 15 => ")", 16 => ")", 17 => ", " . $propValue["max"] . ")");
                if (!isset($propValue["max"])) {
                    $total_code[17] = "";
                ...
                if ($propType["step"] === 0) {
                    ...
                    if ($propValue["step"] === 1 || $propValue["step"] === -1) {
                        $total_code[16] = "";
                        list(, , , , , , , , , , , , $total_code[2], , $total_code[12], , $total_code[14]) = $total_code;
                    } else {
                        if ($propValue["step"] < 0) {
                            $total_code[14] = 0 - $propValue["step"];
                        }
                    }
                    $total_code[4] = "";
                    if (0 < $propValue["step"]) {
                        $total_code[10] = "";
                        list(, , , , , , , , , $total_code[8], $total_code[9]) = $total_code;
                    } else {
                        $total_code[8] = "";
                        list(, , , , , , $total_code[5], $total_code[6], $total_code[7]) = $total_code;
                    }
                }
        ...
        if (isset($namedAttr["total"])) {
            ...
            if (0 < $propType["total"]) {
                $propValue["total"] = (string) $sectionVar . "->value['total']";
            }
        } else {
            if (0 < $propType["total"]) {
                $initLocal["total"] = $propValue["total"];
                $propValue["total"] = (string) $local . "total";
            }
        }
        $cmpFor["iteration"] = (string) $propValue["iteration"] . " <= " . $propValue["total"];
        foreach ($initLocal as $key => $code) {
            $output .= (string) $local . $key . " = " . $code . ";\n";
        }
        $_vars = "array(" . join(", ", $initNamedProperty) . ")";
        $output .= (string) $sectionVar . " = new Smarty_Variable(" . $_vars . ");\n";
        $cond_code = (string) $propValue["total"] . " !== 0";
        if ($propType["total"] === 0) {
        ...
        if (0 < $propType["show"]) {
            $output .= (string) $local . "show = " . $propValue["show"] . " ? " . $cond_code . " : false;\n";
            $output .= "if (" . $local . "show) {\n";
        } else {
            if ($propValue["show"] === "true") {
                $output .= "if (" . $cond_code . ") {\n";
            } else {
                $output .= "if (false) {\n";
            }
        ...
        if (isset($namedAttr["rownum"])) {
            $output .= (string) $sectionVar . "->value['rownum'] = " . $propValue["iteration"] . ";\n";
        }
        if (isset($namedAttr["index_prev"])) {
            $output .= (string) $sectionVar . "->value['index_prev'] = " . $propValue["index"] . " - " . $propValue["step"] . ";\n";
        }
        if (isset($namedAttr["index_next"])) {
            $output .= (string) $sectionVar . "->value['index_next'] = " . $propValue["index"] . " + " . $propValue["step"] . ";\n";
        }
        if (isset($namedAttr["first"])) {
            $output .= (string) $sectionVar . "->value['first'] = (" . $propValue["iteration"] . " === 1);\n";
        }
        if (isset($namedAttr["last"])) {
            $output .= (string) $sectionVar . "->value['last'] = (" . $propValue["iteration"] . " === " . $propValue["total"] . ");\n";
        }
        $output .= "?>";
        return $output;
    }
