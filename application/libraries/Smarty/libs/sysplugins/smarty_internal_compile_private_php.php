...
class Smarty_Internal_Compile_Private_Php extends Smarty_Internal_CompileBase
{
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler, $parameter)
    {
        ...
        if ($_attr["type"] !== "tag") {
            ...
            if ($compiler->php_handling === Smarty::PHP_QUOTE) {
            ...
            if ($compiler->php_handling === Smarty::PHP_PASSTHRU || $_attr["type"] === "unmatched") {
                $compiler->tag_nocache = true;
                $output = addcslashes($_attr["code"], "'\\");
                $compiler->parser->current_buffer->append_subtree($compiler->parser, new Smarty_Internal_ParseTree_Tag($compiler->parser, $compiler->processNocacheCode("<?php echo '" . $output . "';?>", true)));
                return "";
            }
            if ($compiler->php_handling === Smarty::PHP_ALLOW) {
                ...
                if (strpos($lex->value, "<%") === 0) {
                    ...
                    if (strpos($lex->value, "%>") === 0) {
                        ...
                        if (strpos($lex->value, "?>") === 0) {
                            ...
                            if (strpos($lex->value, "<s") === 0) {
                                $lex->phpType = "script";
                                $closeTag = "</script>";
                            } else {
                                if (strpos($lex->value, $lex->smarty->left_delimiter) === 0) {
                                    if ($lex->isAutoLiteral()) {
                                        $lex->token = Smarty_Internal_Templateparser::TP_TEXT;
                                        return NULL;
                                    }
                                    $closeTag = (string) $lex->smarty->left_delimiter . "/php" . $lex->smarty->right_delimiter;
            ...
            while ($body) {
                if (preg_match("~([/][*])|([/][/][^\\n]*)|('[^'\\\\]*(?:\\.[^'\\\\]*)*')|(\"[^\"\\\\]*(?:\\.[^\"\\\\]*)*\")~", $lex->data, $match, PREG_OFFSET_CAPTURE, $start)) {
                    ...
                    if ($close < $pos) {
                        ...
                        if ($phpCommentStart && (!$phpCommentEnd || $close < $pos2)) {
