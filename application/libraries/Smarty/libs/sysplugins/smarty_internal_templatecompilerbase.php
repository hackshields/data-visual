...
abstract class Smarty_Internal_TemplateCompilerBase
{
        ...
        if (!$this->smarty->security_policy || $this->smarty->security_policy->isTrustedPhpFunction($name, $this)) {
            if (strcasecmp($name, "isset") === 0 || strcasecmp($name, "empty") === 0 || strcasecmp($name, "array") === 0 || is_callable($name)) {
                ...
                if ($func_name === "isset") {
                    ...
                    if (in_array($func_name, array("empty", "reset", "current", "end", "prev", "next"))) {
     ...
     * @return bool true if compiling succeeded, false if it failed
     */
    protected abstract function doCompile($_content, $isTemplateSource);
    /**
     * Compile Tag
     *
     * @param  string $tag       tag name
    ...
    private function compileTag2($tag, $args, $parameter)
    {
        ...
        if (($_output = $this->callTagCompiler($tag, $args, $parameter)) === false && (isset($this->parent_compiler->tpl_function[$tag]) || isset($this->template->smarty->ext->_tplFunction) && $this->template->smarty->ext->_tplFunction->getTplFunction($this->template, $tag) !== false)) {
            $args["_attr"]["name"] = "'" . $tag . "'";
            $_output = $this->callTagCompiler("call", $args, $parameter);
        }
        if ($_output !== false) {
            if ($_output !== true && $this->has_code) {
                return $_output;
            }
            return NULL;
        }
        ...
        if (strlen($tag) < 6 || substr($tag, -5) !== "close") {
            ...
            foreach ($this->plugin_search_order as $plugin_type) {
                if ($plugin_type === Smarty::PLUGIN_COMPILER && $this->smarty->loadPlugin("smarty_compiler_" . $tag) && (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag($tag, $this))) {
                    ...
                    if (($function = $this->getPlugin($tag, $plugin_type)) && (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag($tag, $this))) {
            ...
            if (($function = $this->getPlugin($tag, Smarty::PLUGIN_FUNCTION)) && (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag($tag, $this))) {
