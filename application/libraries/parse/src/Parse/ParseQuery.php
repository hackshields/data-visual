...
class ParseQuery
{
    ...
    {
        $this->lessThan($key, array("\$relativeTime" => $relativeTime));
        return $this;
    }
    /**
     * Add a constraint to the query that requires a particular key's value to
    ...
    {
        $this->greaterThan($key, array("\$relativeTime" => $relativeTime));
        return $this;
    }
    /**
     * Add a constraint to the query that requires a particular key's value to
    ...
    {
        $this->greaterThanOrEqualTo($key, array("\$relativeTime" => $relativeTime));
        return $this;
    }
    /**
     * Add a constraint to the query that requires a particular key's value to
    ...
    {
        $this->lessThanOrEqualTo($key, array("\$relativeTime" => $relativeTime));
        return $this;
    }
    /**
    ...
    {
        $this->addCondition($key, "\$text", array("\$search" => array("\$term" => $value)));
        return $this;
    }
    /**
    ...
    public function distinct($key)
    {
        ...
        $result = ParseClient::_request("GET", "aggregate/" . $this->className . "?" . $queryString, $sessionToken, null, true);
    ...
    {
        $this->near($key, $point);
        $this->addCondition($key, "\$maxDistance", $maxDistance / 6371);
        return $this;
    }
    /**
    ...
    {
        $this->addCondition($key, "\$within", array("\$box" => array($southwest, $northeast)));
        return $this;
    }
    /**
     * Add a constraint to the query that requires a particular key's
    ...
    {
        $this->addCondition($key, "\$geoWithin", array("\$polygon" => $points));
        return $this;
    }
    /**
     * Add a constraint to the query that requires a particular key's
    ...
    {
        $this->addCondition($key, "\$geoIntersects", array("\$point" => $point));
        return $this;
    }
    /**
     * Add a constraint to the query that requires a particular key's value to
    ...
    {
        if ($this->orderBy || $this->skip || 0 <= $this->limit) {
    ...
    public function matchesKeyInQuery($key, $queryKey, $query)
    {
        $queryParam = $query->_getOptions();
        $queryParam["className"] = $query->className;
        $this->addCondition($key, "\$select", array("key" => $queryKey, "query" => $queryParam));
        return $this;
    }
    /**
    ...
    public function doesNotMatchKeyInQuery($key, $queryKey, $query)
    {
        $queryParam = $query->_getOptions();
        $queryParam["className"] = $query->className;
        $this->addCondition($key, "\$dontSelect", array("key" => $queryKey, "query" => $queryParam));
        return $this;
    }
    /**
