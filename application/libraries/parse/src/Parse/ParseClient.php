...
final class ParseClient
{
     ...
     * @var bool
     */
    private static $enableCurlExceptions = NULL;
    /**
     * The account key.
     *
     * @var string
     */
    private static $accountKey = NULL;
    /**
     ...
     */
    public static function setHttpClient(HttpClients\ParseHttpable $httpClient)
    {
    ...
    public static function getServerHealth()
    {
        ...
        if ($errorCode) {
            return array("status" => $httpClient->getResponseStatusCode(), "error" => $errorCode, "error_message" => $httpClient->getErrorMessage());
        }
        $status = array("status" => $httpClient->getResponseStatusCode());
        $decoded = json_decode($response, true);
        if (isset($decoded)) {
            $status["response"] = $decoded;
        } else {
            if ($response === "OK") {
                $status["response"] = array("status" => "ok");
            } else {
    ...
    public static function getHttpClient()
    {
        ...
        return function_exists("curl_init") ? new HttpClients\ParseCurlHttpClient() : new HttpClients\ParseStreamHttpClient();
    }
    /**
    ...
    public static function _encode($value, $allowParseObjects)
    {
        ...
        if ($value instanceof Internal\Encodable) {
     ...
     */
    public static function _request($method, $relativeUrl, $sessionToken = NULL, $data = NULL, $useMasterKey = false, $appRequest = false, $contentType = "application/json", $returnHeaders = false)
    {
        if ($data === "[]") {
            $data = "{}";
        }
        ...
        if (!isset($decoded) && $response !== "") {
            throw new ParseException("Bad Request. Could not decode Response: " . "(" . json_last_error() . ") " . self::getLastJSONErrorMsg(), -1);
        }
        if (isset($decoded["error"])) {
            $errorMessage = is_array($decoded["error"]) ? json_encode($decoded["error"]) : $decoded["error"];
            throw new ParseException($errorMessage, isset($decoded["code"]) ? $decoded["code"] : 0);
        }
        if ($returnHeaders) {
    ...
    private static function getLastJSONErrorMsg()
    {
        if (!function_exists("json_last_error_msg")) {
            $error_strings = array(JSON_ERROR_NONE => "No error", JSON_ERROR_DEPTH => "Maximum stack depth exceeded", JSON_ERROR_STATE_MISMATCH => "State mismatch (invalid or malformed JSON)", JSON_ERROR_CTRL_CHAR => "Control character error, potentially incorrectly encoded", JSON_ERROR_SYNTAX => "Syntax error", JSON_ERROR_UTF8 => "Malformed UTF-8 characters, potentially incorrectly encoded");
    ...
    private static function assertServerInitialized()
    {
        if (self::$serverURL === null) {
            throw new \Exception("Missing a valid server url. " . "You must call ParseClient::setServerURL('https://your.parse-server.com', '/parse') " . " before making any requests.");
        }
        if (self::$mountPath === null) {
            throw new \Exception("Missing a valid mount path. " . "You must call ParseClient::setServerURL('https://your.parse-server.com', '/parse') " . " before making any requests.");
        }
    }
    /**
    ...
    private static function assertParseInitialized()
    {
        if (self::$applicationId === null) {
            throw new \Exception("You must call ParseClient::initialize() before making any requests.", 109);
        }
    }
    /**
    ...
    {
        if (self::$accountKey === null || empty($accountKey)) {
            throw new \Exception("You must call ParseClient::initialize(..., \$accountKey) before making any app requests. " . "Your account key must not be null or empty.", 109);
        }
    }
    /**
