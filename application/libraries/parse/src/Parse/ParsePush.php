...
class ParsePush
{
    ...
    public static function send($data, $useMasterKey = false)
    {
        if (isset($data["expiration_time"]) && isset($data["expiration_interval"])) {
            throw new \Exception("Both expiration_time and expiration_interval can't be set.", 138);
        }
        if (isset($data["where"])) {
            if ($data["where"] instanceof ParseQuery) {
                ...
                throw new \Exception("Where parameter for Parse Push must be of type ParseQuery", 111);
            }
        }
        ...
        return ParseClient::_request("POST", "push", null, json_encode(ParseClient::_encode($data, true)), $useMasterKey, false, "application/json", true);
    }
    /**
    ...
    {
        return isset($response["_headers"]) && isset($response["_headers"]["X-Parse-Push-Status-Id"]);
    }
    /**
