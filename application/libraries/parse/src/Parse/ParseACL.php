 ...
 * @package Parse
 */
class ParseACL implements Internal\Encodable
{
    /**
    ...
    private function setAccess($accessType, $userId, $allowed)
    {
        ...
        if (!is_string($userId)) {
            throw new ParseException("Invalid target for access control.", 104);
        }
    ...
    {
        if (!$role->getObjectId()) {
            throw new \Exception("Roles must be saved to the server before they can be used in an ACL.", 104);
        }
    }
    /**
