...
class ParseUser extends ParseObject
{
    ...
    public function signUp()
    {
        ...
        if (!$this->get("password")) {
            throw new ParseException("Cannot sign up user with an empty password.", 201);
        }
        if ($this->getObjectId()) {
            throw new ParseException("Cannot sign up an already existing user.", 208);
        }
        parent::save();
    ...
    public static function logIn($username, $password)
    {
        ...
        if (!$password) {
            throw new ParseException("Cannot log in user with an empty password.", 201);
        }
    ...
    {
        if (!$id) {
            throw new ParseException("Cannot log in Facebook user without an id.", 250);
        }
        if (!$access_token) {
            throw new ParseException("Cannot log in Facebook user without an access token.", 251);
        }
        if (!$expiration_date) {
            $expiration_date = new \DateTime();
            $expiration_date->setTimestamp(time() + 86400 * 60);
        }
        $authData = array("id" => $id, "access_token" => $access_token, "expiration_date" => ParseClient::getProperDateFormat($expiration_date));
     ...
     * @return ParseUser
     */
    public static function logInWithTwitter($id, $screen_name, $consumer_key, $consumer_secret = NULL, $auth_token, $auth_token_secret)
    {
        if (!$id) {
            throw new ParseException("Cannot log in Twitter user without an id.", 250);
        }
        if (!$screen_name) {
            throw new ParseException("Cannot log in Twitter user without Twitter screen name.", 251);
        }
        if (!$consumer_key) {
            throw new ParseException("Cannot log in Twitter user without a consumer key.", 253);
        }
        if (!$auth_token) {
            throw new ParseException("Cannot log in Twitter user without an auth token.", 253);
        }
        if (!$auth_token_secret) {
            throw new ParseException("Cannot log in Twitter user without an auth token secret.", 253);
        }
        $authData = array("id" => $id, "screen_name" => $screen_name, "consumer_key" => $consumer_key, "consumer_secret" => $consumer_secret, "auth_token" => $auth_token, "auth_token_secret" => $auth_token_secret);
    ...
    public static function loginWithAnonymous()
    {
        ...
        $authData = array("id" => $uuid_parts[0] . $uuid_parts[1] . "-" . $uuid_parts[2] . "-" . $uuid_parts[3] . "-" . $uuid_parts[4] . "-" . $uuid_parts[5] . $uuid_parts[6] . $uuid_parts[7]);
    ...
    {
        $data = array("authData" => array($serviceName => $authData));
    ...
    {
        if (!$this->getObjectId()) {
            throw new ParseException("Cannot link an unsaved user, use ParseUser::logInWithFacebook", 104);
        }
        if (!$id) {
            throw new ParseException("Cannot link Facebook user without an id.", 250);
        }
        if (!$access_token) {
            throw new ParseException("Cannot link Facebook user without an access token.", 251);
        }
        if (!$expiration_date) {
            $expiration_date = new \DateTime();
            $expiration_date->setTimestamp(time() + 86400 * 60);
     ...
     * @return ParseUser
     */
    public function linkWithTwitter($id, $screen_name, $consumer_key, $consumer_secret = NULL, $auth_token, $auth_token_secret, $useMasterKey = false)
    {
        if (!$this->getObjectId()) {
    ...
    {
        if (static::$currentUser instanceof $this) {
            return static::$currentUser;
        }
        $storage = ParseClient::getStorage();
        $userData = $storage->get("user");
        if ($userData instanceof $this) {
    ...
    {
        if (static::getCurrentUser() && $this->getObjectId() && $this->getObjectId() == static::getCurrentUser()->getObjectId()) {
            return true;
        }
        return false;
    }
    ...
    public function save($useMasterKey = false)
    {
        if ($this->getObjectId()) {
            ...
            throw new ParseException("You must call signUp to create a new User.", 207);
        }
    }
    /**
    ...
    {
        $json = json_encode(array("email" => $email));
        ParseClient::_request("POST", "verificationEmailRequest", null, $json);
    }
    /**
