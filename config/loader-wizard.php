...
define('SUPPORT_SITE', 'http://support.ioncube.com/');
define('WIZARD_SUPPORT_TICKET_DEPARTMENT', '3');
define('LOADER_FORUM_URL', 'http://forum.ioncube.com/viewforum.php?f=4');
define('LOADERS_FAQ_URL', 'http://www.ioncube.com/faqs/loaders.php');
define('UNIX_ERRORS_URL', 'http://www.ioncube.com/loaders/unix_startup_errors.php');
define('LOADER_WIZARD_URL', LOADERS_PAGE);
define('ENCODER_URL', 'http://www.ioncube.com/sa_encoder.php');
...
define('WIZARD_STATS_URL', 'http://www.ioncube.com/feeds/stats/wizard.php');
define('IONCUBE_DOWNLOADS_SERVER', 'http://downloads3.ioncube.com/loader_downloads');
...
{
    $v = php_version();
    return $v['major'] == 4 && $v['minor'] >= 1 || $v['major'] == 5 && ($v['minor'] >= 1 || $v['release'] >= 3) || $v['major'] == 7;
}
function is_php_version_or_greater($major, $minor, $release = 0)
{
    $version = php_version();
    return $version['major'] > $major || $version['major'] == $major && $version['minor'] > $minor || $version['major'] == $major && $version['minor'] == $minor && $version['release'] >= $release;
}
...
{
    $arch = null;
    $arch_patterns = array('i.?86' => 'x86', 'x86[-_]64' => 'x86', 'x86' => 'x86', 'amd64' => 'x86', 'SMP Tue Jan 01 00:00:00 CEST 2000 all GNU\\/Linux' => 'x86', 'ppc64' => 'ppc', 'ppc' => 'ppc', 'powerpc' => 'ppc', 'sparc' => 'sparc', 'sun' => 'sparc');
...
function is_local()
{
    ...
    if ($_SERVER["SERVER_NAME"] == 'localhost') {
        ...
        if (strpos(':', $ip_address) === false) {
            ...
            $ret = $ip_parts[0] == 10 || $ip_parts[0] == 172 && $ip_parts[1] >= 16 && $ip_parts[1] <= 31 || $ip_parts[0] == 192 && $ip_parts[1] == 168;
        } else {
...
function help_resources($error_list = array())
{
    ...
    $server_type = find_server_type();
    $sysinfo = get_sysinfo();
    $resources = array('<a target="_blank" href="' . 'https://forum.dbface.com' . '">DbFace Forum</a>', '<a target="_blank" href="' . 'https://ticket.dbface.com/open.php' . '">Raise a support ticket through our helpdesk</a>');
...
function initial_page()
{
    ...
    <html>
    <head>
        <title>DbFace ionCube Loader Wizard</title>
...
function loader_not_installed()
{
    ...
    if (!isset($stype)) {
        echo '<p>To use files that have been protected by the ionCube PHP Encoder, a component called the ionCube Loader must be installed.</p>';
    }
...
function heading()
{
    ...
    <div id="header">
        <img src="https://d3krtd5frfbrx5.cloudfront.net/site/website/dbface_logo.png" alt="ionCube logo">
    </div>
    <div id="main">
    <h2>ionCube Loader Wizard for DbFace</h2>
EOT;
}
function footer($update_info = null)
{
    ...
    $year = gmdate("Y");
    echo "</div>";
    echo "<div id=\"footer\">" . "Loader Wizard version " . script_version() . " ";
    if ($update_info === true) {
