...
class AdAsyncRequestSet extends AbstractCrudObject
{
    ...
    public function getRequests(array $fields = array(), array $params = array(), $pending = false)
    {
        ...
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_GET, '/requests', new AdAsyncRequest(), 'EDGE', AdAsyncRequest::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
    ...
    public function getSelf(array $fields = array(), array $params = array(), $pending = false)
    {
        ...
        $param_types = array();
        $enums = array();
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_GET, '/', new AdAsyncRequestSet(), 'NODE', AdAsyncRequestSet::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
        $request->addParams($params);
        $request->addFields($fields);
        return $pending ? $request : $request->execute();
    }
    ...
    {
        $this->assureId();
        $param_types = array('name' => 'string', 'notification_uri' => 'string', 'notification_mode' => 'notification_mode_enum');
        $enums = array('notification_mode_enum' => AdAsyncRequestSetNotificationModeValues::getInstance()->getValues());
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_POST, '/', new AdAsyncRequestSet(), 'NODE', AdAsyncRequestSet::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
        $request->addParams($params);
        $request->addFields($fields);
        return $pending ? $request : $request->execute();
    }
