...
class BusinessTest extends AbstractCrudObjectTestCase implements SkippableFeatureTestInterface
{
    /**
     * @return array
     */
    ...
    public function testCrud()
    {
        ...
        $name = $business->read(array(BusinessFields::NAME))->{BusinessFields::NAME};
        $this->assertCanUpdate($business, array(BusinessFields::NAME => "My Awesome Business"));
        $business->{BusinessFields::NAME} = $name;
        $business->save();
        if ($this->getConfig()->secondaryPageId) {
            $business->claimPage($this->getConfig()->secondaryPageId, BusinessRoles::OWNER);
        ...
        $business->addUserPermissionById($user->{AdUserFields::ID}, BusinessRoles::ADMIN);
        if ($this->getConfig()->secondaryAccountId) {
            $business->claimAdAccount($this->getConfig()->secondaryAccountId, BusinessRoles::AGENCY, array(AdAccountRoles::ADMIN));
            $business->deleteAdAccount($this->getConfig()->secondaryAccountId);
        }
        if ($this->getConfig()->secondaryAppId) {
            $business->claimApp($this->getConfig()->secondaryAppId, BusinessRoles::OWNER);
