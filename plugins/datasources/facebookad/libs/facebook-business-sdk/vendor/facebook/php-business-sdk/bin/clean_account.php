...
$delete_object = function (AbstractCrudObject $object) use($get_class_name) {
    ...
    try {
        ...
        echo sprintf(" > Deletion failed with %s: %s", $get_class_name($e), $e->getMessage()) . PHP_EOL;
    }
};
    ...
    try {
        ...
        foreach ($cursor as $object) {
            if (!$object instanceof AbstractCrudObject) {
                throw new ErrorException(sprintf("Provider returned instance of %s, not a CRUD object", $get_class_name($object)));
            }
        ...
        echo sprintf(" > Edge iteration failed with %s: %s", $get_class_name($e), $e->getMessage()) . PHP_EOL;
    }
};
$account = new AdAccount($config->accountId);
$reflection = new ReflectionClass($account);
foreach ($reflection->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
    if (strncasecmp($method->getName(), 'get', 3) !== 0 || preg_match('/@return\\s+Cursor/', $method->getDocComment()) !== 1) {
        continue;
    }
