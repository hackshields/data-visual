...
class AdsPixelTest extends AbstractCrudObjectTestCase
{
    ...
    public function testCrud()
    {
        ...
        $this->assertCanRead($pixel, array(AdsPixelsFields::NAME));
        ...
        $pixel->update();
        $this->assertCanFetchConnection($pixel, 'getAdAccounts', array(), array('business' => $this->getConfig()->businessId));
        $this->assertCanFetchConnection($pixel, 'getAgencies');
        $this->assertCanFetchConnection($pixel, 'getStats', array(), array('aggregation' => AdsPixelStatAggregations::EVENT));
    ...
    public function testShareWithAdAccount()
    {
        ...
        $pixel = $this->getAccountPixel();
        $pixel->sharePixelWithAdAccount($this->getConfig()->secondaryBusinessId, $account_id_int);
    ...
    public function testShareWithAgency()
    {
        ...
        foreach ($cursor as $agency) {
            if ($agency->{AbstractCrudObject::FIELD_ID} == $this->getConfig()->secondaryBusinessId) {
        ...
        $pixel->sharePixelWithAgency($this->getConfig()->businessId, $this->getConfig()->secondaryBusinessId);
        $pixel->unsharePixelWithAgency($this->getConfig()->businessId, $this->getConfig()->secondaryBusinessId);
    }
}
