...
class AdSetTest extends AbstractCrudObjectTestCase
{
    ...
    public function testCrud()
    {
        ...
        $set->{AdSetFields::CAMPAIGN_ID} = $this->campaign->{CampaignFields::ID};
        ...
        $set->{AdSetFields::START_TIME} = (new \DateTime("+1 week"))->format(\DateTime::ISO8601);
        $set->{AdSetFields::END_TIME} = (new \DateTime("+2 week"))->format(\DateTime::ISO8601);
        $this->assertCanCreate($set, array(AdSet::STATUS_PARAM_NAME => AdSet::STATUS_PAUSED));
        $this->assertCanRead($set);
        $this->assertCanUpdate($set, array(AdSetFields::NAME => $this->getConfig()->testRunId . ' updated'));
