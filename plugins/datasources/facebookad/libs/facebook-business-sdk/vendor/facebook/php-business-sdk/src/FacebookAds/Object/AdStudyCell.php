...
class AdStudyCell extends AbstractCrudObject
{
    ...
    public function getSelf(array $fields = array(), array $params = array(), $pending = false)
    {
        ...
        $param_types = array();
        $enums = array();
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_GET, '/', new AdStudyCell(), 'NODE', AdStudyCell::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
        $request->addParams($params);
        $request->addFields($fields);
        return $pending ? $request : $request->execute();
    }
    ...
    {
        $this->assureId();
        $param_types = array('description' => 'string', 'name' => 'string', 'creation_template' => 'creation_template_enum', 'adaccounts' => 'list<unsigned int>', 'adsets' => 'list<string>', 'campaigns' => 'list<string>');
        $enums = array('creation_template_enum' => AdStudyCellCreationTemplateValues::getInstance()->getValues());
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_POST, '/', new AdStudyCell(), 'NODE', AdStudyCell::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
        $request->addParams($params);
        $request->addFields($fields);
        return $pending ? $request : $request->execute();
    }
