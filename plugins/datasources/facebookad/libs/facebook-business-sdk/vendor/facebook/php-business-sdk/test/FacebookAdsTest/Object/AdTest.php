...
class AdTest extends AbstractCrudObjectTestCase implements SkippableFeatureTestInterface
{
    /**
     * @var Campaign
     */
    ...
    public function testCrudAccess()
    {
        ...
        $group->{AdFields::ADSET_ID} = (int) $this->adSet->{AdSetFields::ID};
        $group->{AdFields::CREATIVE} = array('creative_id' => $this->adCreative->{AdCreativeFields::ID});
        $this->assertCanCreate($group, array(Ad::STATUS_PARAM_NAME => Ad::STATUS_PAUSED));
        $this->assertCanRead($group);
        $this->assertCanUpdate($group, array(AdFields::NAME => $this->getConfig()->testRunId . ' updated'));
        ...
        $this->assertCanFetchConnection($group, 'getAdPreviews', array(), array('ad_format' => 'RIGHT_COLUMN_STANDARD'));
