...
class MediaFingerprint extends AbstractCrudObject
{
    ...
    public function getSelf(array $fields = array(), array $params = array(), $pending = false)
    {
        ...
        $param_types = array();
        $enums = array();
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_GET, '/', new MediaFingerprint(), 'NODE', MediaFingerprint::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
        $request->addParams($params);
        $request->addFields($fields);
        return $pending ? $request : $request->execute();
    }
    ...
    {
        $this->assureId();
        $param_types = array('title' => 'string', 'metadata' => 'Object', 'universal_content_id' => 'string');
        $enums = array();
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_POST, '/', new MediaFingerprint(), 'NODE', MediaFingerprint::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
        $request->addParams($params);
        $request->addFields($fields);
        return $pending ? $request : $request->execute();
    }
