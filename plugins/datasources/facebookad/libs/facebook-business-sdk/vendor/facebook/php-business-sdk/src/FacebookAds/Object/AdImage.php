...
class AdImage extends AbstractCrudObject
{
    ...
    public function getSelf(array $fields = array(), array $params = array(), $pending = false)
    {
        ...
        $param_types = array();
        $enums = array();
        $request = new ApiRequest($this->api, $this->data['id'], RequestInterface::METHOD_GET, '/', new AdImage(), 'NODE', AdImage::getFieldsEnum()->getValues(), new TypeChecker($param_types, $enums));
    ...
    public function create(array $params = array())
    {
        ...
        $request = $this->getApi()->prepareRequest('/' . $this->assureParentId() . '/' . $this->getEndpoint(), RequestInterface::METHOD_POST, $params);
        ...
        $this->data[static::FIELD_ID] = substr($this->getParentId(), 4) . ':' . $this->data[AdImageFields::HASH];
        return $this;
    }
    /**
     * Uploads images from a zip file and returns a cursor of results
    ...
    protected function arrayFromZip($params = array())
    {
        ...
        foreach ($content['images'] as $image) {
            $adimage = new AdImage(substr($this->getParentId(), 4) . ':' . $image[AdImageFields::HASH], $this->getParentId(), $this->getApi());
