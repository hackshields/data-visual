...
class CursorTest extends AbstractUnitTestCase
{
    ...
    {
        $content = array('paging' => array('cursors' => array('after' => $this->getUniquePageId(), 'before' => $this->getUniquePageId()))) + $this->createEmptyResponseContent();
        $count = rand(2, 5);
        for ($i = 0; $i < $count; $i++) {
            $content['data'][] = array(AbstractCrudObject::FIELD_ID => $i);
        }
        return $content;
    }
    /**
    ...
    {
        return Request::PROTOCOL_HTTPS . Client::DEFAULT_LAST_LEVEL_DOMAIN . '.' . Client::DEFAULT_GRAPH_BASE_DOMAIN . '/node/edge';
    }
    /**
     * @param int $num_pages
    ...
    {
        $response = $this->createResponseMock();
        $response->method('getContent')->willReturn(array('data' => array()));
        return $response;
    }
    ...
    {
        return array(array(array()), array(array('data' => null)), array(array('data' => 1)));
    }
    /**
    ...
    {
        $cursor = new Cursor($this->createResponseChainMock(1), $this->objectPrototype);
