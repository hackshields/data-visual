...
{
    protected function assertClusterChangesResponse(CustomAudience $ca, array $users, $response)
    {
    ...
    public function testCustomAudiences()
    {
        ...
        $this->assertCanUpdate($ca, array(CustomAudienceFields::NAME => $this->getConfig()->testRunId . ' updated'));
    ...
    public function testAppIdsPayload()
    {
        ...
        $add = $ca->addUsers($users, CustomAudienceTypes::ID, array($this->getConfig()->appId));
        $this->assertClusterChangesResponse($ca, $users, $add);
        $remove = $ca->removeUsers($users, CustomAudienceTypes::ID, array($this->getConfig()->appId));
        $this->assertClusterChangesResponse($ca, $users, $remove);
        $optout = $ca->optOutUsers($users, CustomAudienceTypes::ID, array($this->getConfig()->appId));
        $this->assertSuccessResponse($optout);
        $ca->deleteSelf();
    }
}
