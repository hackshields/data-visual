...
class PropertyNormalizerTest extends TestCase
{
    ...
    public function testNormalize()
    {
        ...
        $this->assertEquals(['foo' => 'foo', 'bar' => 'bar', 'camelCase' => 'camelcase'], $this->normalizer->normalize($obj, 'any'));
    }
    public function testDenormalize()
    {
        $obj = $this->normalizer->denormalize(['foo' => 'foo', 'bar' => 'bar'], __NAMESPACE__ . '\\PropertyDummy', 'any');
    ...
    public function testNormalizeWithParentClass()
    {
        ...
        $this->assertEquals(['foo' => 'foo', 'bar' => 'bar', 'kevin' => 'Kevin', 'coopTilleuls' => 'coop', 'fooBar' => null, 'symfony' => null, 'baz' => 'baz'], $this->normalizer->normalize($group, 'any'));
    }
    public function testDenormalizeWithParentClass()
    {
        $obj = $this->normalizer->denormalize(['foo' => 'foo', 'bar' => 'bar', 'kevin' => 'Kevin', 'baz' => 'baz'], GroupDummyChild::class, 'any');
    ...
    {
        $obj = $this->normalizer->denormalize(['foo' => 'foo', 'bar' => 'bar'], __NAMESPACE__ . '\\PropertyConstructorDummy', 'any');
        $this->assertEquals('foo', $obj->getFoo());
        $this->assertEquals('bar', $obj->getBar());
    }
    public function testConstructorDenormalizeWithNullArgument()
    {
        $obj = $this->normalizer->denormalize(['foo' => null, 'bar' => 'bar'], __NAMESPACE__ . '\\PropertyConstructorDummy', '
            any');
    ...
    private function doTestIgnoredAttributes(bool $legacy = false)
    {
        ...
        $obj->foo = 'foo';
        $obj->setBar('bar');
        $this->assertEquals([], $this->normalizer->normalize($obj, 'any'));
    }
    public function testGroupsNormalize()
    {
        ...
        $this->assertEquals(['bar' => 'bar'], $this->normalizer->normalize($obj, null, [PropertyNormalizer::GROUPS => ['c']]));
        // The PropertyNormalizer is also able to hydrate properties from parent classes
        $this->assertEquals(['symfony' => 'symfony', 'foo' => 'foo', 'fooBar' => 'fooBar', 'bar' => 'bar', 'kevin' => 'kevin', 'coopTilleuls' => 'coopTilleuls'], $this->normalizer->normalize($obj, null, [PropertyNormalizer::GROUPS => ['a', 'c']]));
    }
    public function testGroupsDenormalize()
    {
        ...
        $normalized = $this->normalizer->denormalize($toNormalize, 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, [PropertyNormalizer::GROUPS => ['a']]);
        $this->assertEquals($obj, $normalized);
        $obj->setBar('bar');
        $normalized = $this->normalizer->denormalize($toNormalize, 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, [PropertyNormalizer::GROUPS => ['a', 'b']]);
        $this->assertEquals($obj, $normalized);
    }
    public function testGroupsNormalizeWithNameConverter()
    {
        ...
        $this->assertEquals(['bar' => null, 'foo_bar' => '@dunglas', 'symfony' => '@coopTilleuls'], $this->normalizer->normalize($obj, null, [PropertyNormalizer::GROUPS => ['name_converter']]));
    }
    public function testGroupsDenormalizeWithNameConverter()
    {
        ...
        $obj->setFooBar('@dunglas');
        $obj->setSymfony('@coopTilleuls');
        $this->assertEquals($obj, $this->normalizer->denormalize(['bar' => null, 'foo_bar' => '@dunglas', 'symfony' => '@coopTilleuls', 'coop_tilleuls' => 'les-tilleuls.coop'], 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, [PropertyNormalizer::GROUPS => ['name_converter']]));
    }
    public function provideCallbacks()
    {
        return [[['bar' => function ($bar) {
            return 'baz';
        }], 'baz', ['foo' => '', 'bar' => 'baz'], 'Change a string'], [['bar' => function ($bar) {
        }], 'baz', ['foo' => '', 'bar' => null], 'Null an item'], [['bar' => function ($bar) {
            return $bar->format('d-m-Y H:i:s');
        }], new \DateTime('2011-09-10 06:30:00'), ['foo' => '', 'bar' => '10-09-2011 06:30:00'], 'Format a date'], [['bar' => function ($bars) {
            $foos = '';
            foreach ($bars as $bar) {
                $foos .= $bar->getFoo();
            }
            return $foos;
        }], [new PropertyConstructorDummy('baz', ''), new PropertyConstructorDummy('quux', '')], ['foo' => '', 'bar' => 'bazquux'], 'Collect a property'], [['bar' => function ($bars) {
            return \count($bars);
        }], [new PropertyConstructorDummy('baz', ''), new PropertyConstructorDummy('quux', '')], ['foo' => '', 'bar' => 2], 'Count a property']];
    }
    /**
    ...
    {
        $this->assertEquals(new PropertyDummy(), $this->normalizer->denormalize(['non_existing' => true], __NAMESPACE__ . '\\PropertyDummy'));
    }
    ...
    public function testMaxDepth()
    {
        ...
        $expected = ['foo' => 'level1', 'child' => ['foo' => 'level2', 'child' => ['child' => null, 'bar' => null], 'bar' => null], 'bar' => null];
