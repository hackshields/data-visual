...
class ObjectNormalizerTest extends TestCase
{
    ...
    public function testNormalize()
    {
        ...
        $this->assertEquals(['foo' => 'foo', 'bar' => 'bar', 'baz' => true, 'fooBar' => 'foobar', 'camelCase' => 'camelcase', 'object' => 'string_object'], $this->normalizer->normalize($obj, 'any'));
    }
    public function testDenormalize()
    {
        $obj = $this->normalizer->denormalize(['foo' => 'foo', 'bar' => 'bar', 'baz' => true, 'fooBar' => 'foobar'], __NAMESPACE__ . '\\ObjectDummy', 'any');
    ...
    {
        $obj = $this->normalizer->denormalize(['foo' => 'foo', 'bar' => 'bar', 'baz' => true, 'fooBar' => 'foobar'], __NAMESPACE__ . '\\ObjectConstructorDummy', 'any');
        $this->assertEquals('foo', $obj->getFoo());
        $this->assertEquals('bar', $obj->bar);
        $this->assertTrue($obj->isBaz());
    }
    ...
    {
        $obj = $this->normalizer->denormalize(['foo' => 'foo', 'bar' => null, 'baz' => true], __NAMESPACE__ . '\\ObjectConstructorDummy', 'any');
        $this->assertEquals('foo', $obj->getFoo());
    ...
    {
        $obj = $this->normalizer->denormalize(['foo' => 'test', 'baz' => [1, 2, 3]], __NAMESPACE__ . '\\ObjectConstructorOptionalArgsDummy', 'any');
    ...
    {
        $obj = $this->normalizer->denormalize(['bar' => 'test'], __NAMESPACE__ . '\\ObjectConstructorArgsWithDefaultValueDummy', 'any');
    ...
    {
        $data = ['id' => 10, 'inner' => ['foo' => 'oof', 'bar' => 'rab']];
    ...
    {
        $data = ['id' => 10, 'unknown' => ['foo' => 'oof', 'bar' => 'rab']];
        $normalizer = new ObjectNormalizer();
        $serializer = new Serializer([$normalizer]);
        $normalizer->setSerializer($serializer);
    ...
    public function testFillWithEmptyDataWhenMissingData()
    {
        $data = ['foo' => 10];
        $normalizer = new ObjectNormalizer();
        $result = $normalizer->denormalize($data, DummyValueObject::class, 'json', ['default_constructor_arguments' => [DummyValueObject::class => ['foo' => '', 'bar' => '', 'baz' => null]]]);
    ...
    public function testGroupsNormalize()
    {
        ...
        $this->assertEquals(['bar' => 'bar'], $this->normalizer->normalize($obj, null, [ObjectNormalizer::GROUPS => ['c']]));
        $this->assertEquals(['symfony' => 'symfony', 'foo' => 'foo', 'fooBar' => 'fooBar', 'bar' => 'bar', 'kevin' => 'kevin', 'coopTilleuls' => 'coopTilleuls'], $this->normalizer->normalize($obj, null, [ObjectNormalizer::GROUPS => ['a', 'c']]));
    }
    public function testGroupsDenormalize()
    {
        ...
        $normalized = $this->normalizer->denormalize($toNormalize, 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, [ObjectNormalizer::GROUPS => ['a']]);
        $this->assertEquals($obj, $normalized);
        $obj->setBar('bar');
        $normalized = $this->normalizer->denormalize($toNormalize, 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, [ObjectNormalizer::GROUPS => ['a', 'b']]);
        $this->assertEquals($obj, $normalized);
    }
    ...
    public function testGroupsNormalizeWithNameConverter()
    {
        ...
        $this->assertEquals(['bar' => null, 'foo_bar' => '@dunglas', 'symfony' => '@coopTilleuls'], $this->normalizer->normalize($obj, null, [ObjectNormalizer::GROUPS => ['name_converter']]));
    }
    public function testGroupsDenormalizeWithNameConverter()
    {
        ...
        $obj->setFooBar('@dunglas');
        $obj->setSymfony('@coopTilleuls');
        $this->assertEquals($obj, $this->normalizer->denormalize(['bar' => null, 'foo_bar' => '@dunglas', 'symfony' => '@coopTilleuls', 'coop_tilleuls' => 'les-tilleuls.coop'], 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\GroupDummy', null, [ObjectNormalizer::GROUPS => ['name_converter']]));
    }
    public function testMetadataAwareNameConvertorWithNotSerializedConstructorParameter()
    {
        ...
        $obj->setBar('xyz');
        $this->assertEquals($obj, $this->normalizer->denormalize(['bar' => 'xyz'], 'Symfony\\Component\\Serializer\\Tests\\Fixtures\\NotSerializedConstructorArgumentDummy', null, [ObjectNormalizer::DEFAULT_CONSTRUCTOR_ARGUMENTS => ['Symfony\\Component\\Serializer\\Tests\\Fixtures\\NotSerializedConstructorArgumentDummy' => ['foo' => 'buz']]]));
    }
    /**
    ...
    private function doTestIgnoredAttributes(bool $legacy = false)
    {
        ...
        $obj->bar = 'bar';
        $obj->setBaz(true);
        $this->assertEquals(['fooBar' => 'foobar'], $this->normalizer->normalize($obj, 'any'));
    }
    ...
    private function doTestIgnoredAttributesDenormalize(bool $legacy = false)
    {
        ...
        $obj = new ObjectDummy();
        $obj->setFoo('foo');
        $this->assertEquals($obj, $this->normalizer->denormalize(['fooBar' => 'fooBar', 'foo' => 'foo', 'baz' => 'baz'], __NAMESPACE__ . '\\ObjectDummy'));
    }
    public function provideCallbacks()
    {
        return [[['bar' => function ($bar) {
            ...
            return 'baz';
        }], 'baz', ['foo' => '', 'bar' => 'baz', 'baz' => true], 'Change a string'], [['bar' => function ($value, $object, $attributeName, $format, $context) {
        ...
        }], 'baz', ['foo' => '', 'bar' => null, 'baz' => true], 'Null an item'], [['bar' => function ($bar) {
            return $bar->format('d-m-Y H:i:s');
        }], new \DateTime('2011-09-10 06:30:00'), ['foo' => '', 'bar' => '10-09-2011 06:30:00', 'baz' => true], 'Format a date'], [['bar' => function ($bars) {
            $foos = '';
            foreach ($bars as $bar) {
                $foos .= $bar->getFoo();
            }
            return $foos;
        }], [new ObjectConstructorDummy('baz', '', false), new ObjectConstructorDummy('quux', '', false)], ['foo' => '', 'bar' => 'bazquux', 'baz' => true], 'Collect a property'], [['bar' => function ($bars) {
            return \count($bars);
        }], [new ObjectConstructorDummy('baz', '', false), new ObjectConstructorDummy('quux', '', false)], ['foo' => '', 'bar' => 2, 'baz' => true], 'Count a property']];
    }
    /**
    ...
    public function testSiblingReference()
    {
        ...
        $expected = ['sibling0' => ['coopTilleuls' => 'Les-Tilleuls.coop'], 'sibling1' => ['coopTilleuls' => 'Les-Tilleuls.coop'], 'sibling2' => ['coopTilleuls' => 'Les-Tilleuls.coop']];
    ...
    {
        $this->assertEquals(new ObjectDummy(), $this->normalizer->denormalize(['non_existing' => true], __NAMESPACE__ . '\\ObjectDummy'));
    }
    ...
    public function testNormalizeNotSerializableContext()
    {
        ...
        $expected = ['foo' => null, 'baz' => null, 'fooBar' => '', 'camelCase' => null, 'object' => null, 'bar' => null];
    ...
    private function doTestMaxDepth(bool $legacy = false)
    {
        ...
        $expected = ['bar' => null, 'foo' => 'level1', 'child' => ['bar' => null, 'foo' => 'level2', 'child' => ['bar' => null, 'child' => null]]];
        $this->assertEquals($expected, $result);
        $expected = ['bar' => null, 'foo' => 'level1', 'child' => ['bar' => null, 'foo' => 'level2', 'child' => ['bar' => null, 'child' => null, 'foo' => 'handler']]];
    ...
    public function testDenomalizeRecursive()
    {
        ...
        $obj = $serializer->denormalize(['inner' => ['foo' => 'foo', 'bar' => 'bar'], 'date' => '1988/01/21', 'inners' => [['foo' => 1], ['foo' => 2]]], ObjectOuter::class);
    ...
    public function testAttributesContextNormalize()
    {
        ...
        $this->assertEquals(['foo' => 'foo', 'baz' => true, 'object' => ['foo' => 'innerFoo']], $serializer->normalize($objectDummy, null, $context));
        $context = ['attributes' => ['foo', 'baz', 'object']];
        $this->assertEquals(['foo' => 'foo', 'baz' => true, 'object' => ['foo' => 'innerFoo', 'bar' => 'innerBar']], $serializer->normalize($objectDummy, null, $context));
    }
    public function testAttributesContextDenormalize()
    {
        ...
        $this->assertEquals($objectOuter, $serializer->denormalize(['foo' => 'foo', 'bar' => 'bar', 'date' => '2017-02-03', 'inner' => ['foo' => 'innerFoo', 'bar' => 'innerBar']], ObjectOuter::class, null, $context));
    }
    public function testAttributesContextDenormalizeConstructor()
    {
        ...
        $this->assertEquals($obj, $serializer->denormalize(['foo' => 'b', 'inner' => ['foo' => 'foo', 'bar' => 'bar']], DummyWithConstructorObjectAndDefaultValue::class, null, $context));
    }
    public function testNormalizeSameObjectWithDifferentAttributes()
    {
        ...
        $this->assertEquals(['foo' => ['bar' => 'foo.bar'], 'bar' => ['foo' => 'bar.foo']], $this->normalizer->normalize($dummy, 'json', ['attributes' => ['foo' => ['bar'], 'bar' => ['foo']]]));
    }
