...
class SerializerTest extends TestCase
{
    ...
    {
        $serializer = new Serializer([new GetSetMethodNormalizer(), new PropertyNormalizer(), new ObjectNormalizer(), new CustomNormalizer(), new ArrayDenormalizer()], ['json' => new JsonEncoder()]);
        $this->assertTrue($serializer->supportsDenormalization([], __NAMESPACE__ . '\\Model[]', 'json'));
    }
    public function testDeserializeArray()
    {
        $jsonData = '[{"title":"foo","numbers":[5,3]},{"title":"bar","numbers":[2,8]}]';
        $expectedData = [Model::fromArray(['title' => 'foo', 'numbers' => [5, 3]]), Model::fromArray(['title' => 'bar', 'numbers' => [2, 8]])];
        $serializer = new Serializer([new GetSetMethodNormalizer(), new ArrayDenormalizer()], ['json' => new JsonEncoder()]);
    ...
    public function testNormalizerAware()
    {
        ...
        $normalizerAware->expects($this->once())->method('setNormalizer')->with($this->isInstanceOf(NormalizerInterface::class));
    ...
    public function testDenormalizerAware()
    {
        ...
        $denormalizerAware->expects($this->once())->method('setDenormalizer')->with($this->isInstanceOf(DenormalizerInterface::class));
    ...
    public function testDeserializeAndSerializeAbstractObjectsWithTheClassMetadataDiscriminatorResolver()
    {
        ...
        $loaderMock->method('hasMetadataFor')->will($this->returnValueMap([[AbstractDummy::class, true]]));
        $loaderMock->method('getMetadataFor')->will($this->returnValueMap([[AbstractDummy::class, new ClassMetadata(AbstractDummy::class, new ClassDiscriminatorMapping('type', ['first' => AbstractDummyFirstChild::class, 'second' => AbstractDummySecondChild::class]))]]));
    ...
    public function testDeserializeAndSerializeInterfacedObjectsWithTheClassMetadataDiscriminatorResolverAndGroups()
    {
        ...
        $example->two = 2;
        $serializer = $this->serializerWithClassDiscriminator();
        $deserialized = $serializer->deserialize('{"type":"one","one":1,"two":2}', DummyMessageInterface::class, 'json', ['groups' => ['two']]);
        $this->assertEquals($example, $deserialized);
        $serialized = $serializer->serialize($deserialized, 'json', ['groups' => ['two']]);
