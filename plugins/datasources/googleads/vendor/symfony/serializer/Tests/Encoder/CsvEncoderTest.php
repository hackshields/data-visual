...
, $this->encoder->encode($value, 'csv'));
    ...
    {
        $value = [['foo' => 'hello', 'bar' => 'hey ho'], ['foo' => 'hi', 'bar' => 'let\'s go']];
        $this->assertEquals(<<<'CSV'
foo,bar
hello,"hey ho"
hi,"let's go"
...
, $this->encoder->encode('foo', 'csv'));
    ...
    {
        $value = ['foo' => 'hello', 'bar' => [['id' => 'yo', 1 => 'wesh'], ['baz' => 'Halo', 'foo' => 'olá']]];
        $this->assertEquals(<<<'CSV'
...
CSV
, $this->encoder->encode($value, 'csv', [CsvEncoder::DELIMITER_KEY => ';', CsvEncoder::ENCLOSURE_KEY => "'", CsvEncoder::ESCAPE_CHAR_KEY => '|', CsvEncoder::KEY_SEPARATOR_KEY => '-']));
    }
    ...
    {
        $value = [['a' => ['foo', 'bar']], ['a' => [], 'b' => 'baz'], ['a' => ['bar', 'foo'], 'c' => 'pong']];
        $csv = <<<CSV
a.0,a.1,c,b
foo,bar,,
,,,baz
...
CSV;
    ...
    {
        $context = [CsvEncoder::HEADERS_KEY => ['b', 'c']];
        $value = [['a' => 'foo', 'b' => 'bar']];
        $csv = <<<CSV
b,c,a
bar,,foo
...
CSV
, $this->encoder->encode(['=2+3'], 'csv', [CsvEncoder::ESCAPE_FORMULAS_KEY => true]));
        $this->assertSame(<<<'CSV'
0
"	-2+3"
...
CSV
, $this->encoder->encode(['@MyDataColumn'], 'csv', [CsvEncoder::ESCAPE_FORMULAS_KEY => true]));
    }
...
, 'csv', [CsvEncoder::AS_COLLECTION_KEY => false]));
    ...
    {
        $expected = [['foo' => 'a', 'bar' => 'b'], ['foo' => 'c', 'bar' => 'd'], ['foo' => 'f']];
        $this->assertEquals($expected, $this->encoder->decode(<<<'CSV'
foo,bar
a,b
c,d
...
, 'csv', [CsvEncoder::AS_COLLECTION_KEY => true]));
    ...
    {
        $expected = [['foo' => 'bar', 'relations' => [['a' => 'b'], ['a' => 'b']]], ['foo' => 'bat', 'relations' => [['a' => 'b'], ['a' => '']]], ['foo' => 'bat', 'relations' => [['a' => 'b']]], ['foo' => 'baz', 'relations' => [['a' => 'c'], ['a' => 'c']]]];
        $this->assertEquals($expected, $this->encoder->decode(<<<'CSV'
...
, 'csv'));
    ...
    {
        $expected = [['foo' => 'a', 'bar' => ['baz' => ['bat' => 'b']]], ['foo' => 'c', 'bar' => ['baz' => ['bat' => 'd']]]];
        $this->assertEquals($expected, $this->encoder->decode(<<<'CSV'
foo,bar.baz.bat
a,b
c,d
...
'hell''o';b;c
CSV
, 'csv', [CsvEncoder::DELIMITER_KEY => ';', CsvEncoder::ENCLOSURE_KEY => "'", CsvEncoder::ESCAPE_CHAR_KEY => '|', CsvEncoder::KEY_SEPARATOR_KEY => '-', CsvEncoder::AS_COLLECTION_KEY => true]));
    }
