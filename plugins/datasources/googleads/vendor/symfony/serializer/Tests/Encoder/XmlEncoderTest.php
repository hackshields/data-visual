...
class XmlEncoderTest extends TestCase
{
    ...
    {
        $obj = new ScalarDummy();
        $obj->xmlFoo = ['foo-bar' => ['@id' => 1, '@name' => 'Bar'], 'Foo' => ['Bar' => 'Test', '@Type' => 'test'], 'föo_bär' => 'a', 'Bar' => [1, 2, 3], 'a' => 'b'];
    ...
    {
        $obj = new ScalarDummy();
        $obj->xmlFoo = ['foo-bar' => 'a', 'foo_bar' => 'a', 'föo_bär' => 'a'];
    ...
    public function testEncodeXmlAttributes()
    {
        ...
        $context = ['xml_version' => '1.1', 'xml_encoding' => 'utf-8', 'xml_standalone' => true];
...
XML;
        $this->assertSame(['@index' => 1.8207724176001167E+59, '#' => 'Name'], $this->encoder->decode($source, 'xml'));
    }
...
XML;
        ...
        $expected = ['@a' => '018', '@b' => '-12.11', 'node' => ['@a' => '018', '@b' => '-12.11', '#' => '']];
...
XML;
    ...
    public function testDecodeRootAttributes()
    {
        ...
        $expected = ['firstname' => 'Peter', 'lastname' => 'Mac Calloway', '@eye-color' => 'brown'];
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
    public function testDecodeArray()
    {
        ...
        $expected = ['people' => ['person' => [['firstname' => 'Benjamin', 'lastname' => 'Alexandre'], ['firstname' => 'Damien', 'lastname' => 'Clay']]]];
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
...
XML;
        $expected = ['person' => [['firstname' => 'Benjamin', 'lastname' => 'Alexandre'], ['firstname' => 'Damien', 'lastname' => 'Clay']]];
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
...
XML;
        if ($legacy) {
            ...
            $this->encoder = new XmlEncoder([XmlEncoder::ROOT_NODE_NAME => 'people', XmlEncoder::DECODER_IGNORED_NODE_TYPES => [XML_PI_NODE]]);
        }
        $serializer = new Serializer([new CustomNormalizer()], ['xml' => new XmlEncoder()]);
        $this->encoder->setSerializer($serializer);
        $expected = ['person' => [['firstname' => 'Benjamin', 'lastname' => 'Alexandre', '#comment' => ' This comment should be decoded. '], ['firstname' => 'Damien', 'lastname' => 'Clay']]];
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml'));
    }
...
XML;
        $expected = ['@nodeType' => 'order_row', '@virtualEntity' => 'true', 'order_row' => [['id' => [16], 'test' => [16]]]];
        $this->assertEquals($expected, $this->encoder->decode($source, 'xml', ['as_collection' => true]));
    }
    public function testDecodeWithoutItemHash()
    {
        $obj = new ScalarDummy();
        $obj->xmlFoo = ['foo-bar' => ['@key' => 'value', 'item' => ['@key' => 'key', 'key-val' => 'val']], 'Foo' => ['Bar' => 'Test', '@Type' => 'test'], 'föo_bär' => 'a', 'Bar' => [1, 2, 3], 'a' => 'b'];
        $expected = ['foo-bar' => ['@key' => 'value', 'key' => ['@key' => 'key', 'key-val' => 'val']], 'Foo' => ['Bar' => 'Test', '@Type' => 'test'], 'föo_bär' => 'a', 'Bar' => [1, 2, 3], 'a' => 'b'];
    ...
    {
        return ['@xmlns' => 'http://www.w3.org/2005/Atom', '@xmlns:app' => 'http://www.w3.org/2007/app', '@xmlns:media' => 'http://search.yahoo.com/mrss/', '@xmlns:gd' => 'http://schemas.google.com/g/2005', '@xmlns:yt' => 'http://gdata.youtube.com/schemas/2007', 'qux' => '1', 'app:foo' => 'foo', 'yt:bar' => ['a', 'b'], 'media:baz' => ['media:key' => 'val', 'media:key2' => 'val', 'A B' => 'bar', 'item' => [['title' => 'title1'], ['title' => 'title2']], 'Barry' => ['@size' => 'large', 'FooBar' => ['Baz' => 'Ed', '@gd:id' => 1]]]];
    }
...
XML;
    ...
    private function doTestEncodeWithoutPi(bool $legacy = false)
    {
        if ($legacy) {
            ...
            $encoder = new XmlEncoder([XmlEncoder::ROOT_NODE_NAME => 'response', XmlEncoder::ENCODER_IGNORED_NODE_TYPES => [XML_PI_NODE]]);
        }
        $expected = '<response/>';
    ...
    private function doTestEncodeWithoutComment(bool $legacy = false)
    {
        if ($legacy) {
            ...
            $encoder = new XmlEncoder([XmlEncoder::ROOT_NODE_NAME => 'response', XmlEncoder::ENCODER_IGNORED_NODE_TYPES => [XML_COMMENT_NODE]]);
        }
        $expected = <<<'XML'
<?xml version="1.0"?>
<response/>
...
XML;
    ...
    private function createMockDateTimeNormalizer()
    {
        ...
        $mock->expects($this->once())->method('normalize')->with(new \DateTime($this->exampleDateTimeString), 'xml', [])->willReturn($this->exampleDateTimeString);
        $mock->expects($this->once())->method('supportsNormalization')->with(new \DateTime($this->exampleDateTimeString), 'xml')->willReturn(true);
        return $mock;
    }
    /**
     * @return string
