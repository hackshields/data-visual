...
class Pql
{
    ...
    public static function createValue($value)
    {
        if ($value instanceof Value) {
            ...
            throw new InvalidArgumentException(sprintf('Unsupported value type [%s]', is_object($value) ? get_class($value) : gettype($value)));
        }
    }
    /**
    ...
    public static function toString(Value $value)
    {
        if ($value instanceof BooleanValue) {
            ...
            return $value->getValue() !== null ? AdManagerDateTimes::toDateTimeString($value->getValue()) : '';
     ...
     */
    public static function combineResultSets(ResultSet $first, ResultSet $second)
    {
        $firstColumns = self::getColumnLabels($first);
        $secondColumns = self::getColumnLabels($second);
        if ($firstColumns !== $secondColumns) {
            throw new InvalidArgumentException(sprintf('First result set columns [%s] do not match second result ' . 'set columns [%s]', implode(', ', $firstColumns), implode(', ', $secondColumns)));
        }
     ...
     *     set
     */
    public static function resultSetTo2DimensionStringArray(ResultSet $resultSet)
    {
        $array2D = [];
