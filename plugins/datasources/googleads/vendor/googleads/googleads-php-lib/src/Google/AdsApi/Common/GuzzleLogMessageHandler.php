        ...
        return function (callable $handler) use($logger, $messageFormatter) {
            return function ($request, array $options) use($handler, $logger, $messageFormatter) {
                return $handler($request, $options)->then(function ($response) use($request, $logger, $messageFormatter) {
                    ...
                    $cachedResponse = $response->withBody(new CachingStream($response->getBody()));
                    if (!$logger instanceof Logger || $logger->isHandling(Logger::DEBUG)) {
                        ...
                        $logger->debug($messageFormatter->formatDetailed($request, $cachedResponse));
                    }
                    ...
                    // stream.
                    $cachedResponse = $response->withBody(new CachingStream($response->getBody()));
                    $newException = RequestException::create($request, $cachedResponse, $reason);
                    if (!$logger instanceof Logger || $logger->isHandling(Logger::NOTICE)) {
                        // formatDetailed() can produce long log messages
                        // so check if it handles NOTICE level when
                        // possible.
                        $logger->notice($messageFormatter->formatDetailed($request, $cachedResponse, $reason));
                    }
                    // Rewind the response stream so it can be returned as a
                    // report download result later.
                    \GuzzleHttp\Psr7\rewind_body($cachedResponse);
