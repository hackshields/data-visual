...
class ReportDownloader
{
     ...
     *     HTTP calls
     */
    public function __construct(ReportService $reportService, $reportJobId, $pollTimeSeconds = null, Client $httpClient = null, GuzzleHttpClientFactory $httpClientFactory = null)
    {
        ...
        if ($httpClientFactory === null) {
            $logMessageFormatterProvider = new AdManagerGuzzleLogMessageFormatterProvider($reportService->getAdsSession(), false, self::$REDACTED_DATA_MESSAGE);
            $httpClientFactory = new AdsGuzzleHttpClientFactory($this->logger, $logMessageFormatterProvider->getGuzzleLogMessageFormatter(), $httpClient);
        }
    ...
    public function waitForReportToFinish()
    {
        ...
        while ($reportJobStatus === ReportJobStatus::IN_PROGRESS) {
            $this->logger->info(sprintf('Report job ID %d has status %s. Sleeping for %d seconds ' . 'before polling again for report status.', $this->reportJobId, $reportJobStatus, $this->pollTimeSeconds));
    ...
    public function downloadReport($exportFormat, $filePath = null)
    {
        ...
        $requestOptions = [];
        $requestOptions[RequestOptions::HEADERS] = ['User-Agent' => $this->getFormattedUserAgent()];
    ...
    private function getDownloadUrl($exportFormat)
    {
        ...
        if ($reportJobStatus === ReportJobStatus::IN_PROGRESS) {
            throw new UnexpectedValueException(sprintf('Report %d must be completed before downloading. ' . 'Report is still %s.', $this->reportJobId, ReportJobStatus::IN_PROGRESS));
        } elseif ($reportJobStatus === ReportJobStatus::FAILED) {
            throw new UnexpectedValueException(sprintf('Cannot download report %d because it has a status of %s.', $this->reportJobId, ReportJobStatus::FAILED));
        }
    ...
    private function getFormattedUserAgent()
    {
        ...
        return $session->getAdsHeaderFormatter()->formatApplicationNameForGuzzleHeader($session->getApplicationName(), AdManagerHeaderHandler::PRODUCT_NAME_FOR_SOAP_HEADER, false);
    }
}
