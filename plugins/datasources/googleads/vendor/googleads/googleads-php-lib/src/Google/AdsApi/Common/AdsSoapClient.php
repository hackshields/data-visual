...
class AdsSoapClient extends SoapClient
{
    ...
    private static $SOAP_FAULT_NAME_TO_TYPE_MAP = ['ApiExceptionFault' => 'ApiException'];
    ...
    public function __soapCall($function_name, $arguments, $options = null, $input_headers = null, &$output_headers = null)
    {
        ...
        $existingStreamContextOptions['http']['header'] = implode(
            ...
            array_map(function ($headerName, $headerValue) {
                return sprintf('%s: %s', $headerName, $headerValue);
            }, array_keys($httpHeaders), $httpHeaders)
        );
        stream_context_set_option($this->streamContext, $existingStreamContextOptions);
        // Generate the SOAP headers for this API request.
        $input_headers[] = $this->headerHandler->generateSoapHeaders($this->adsSession, $this->serviceDescriptor);
        $response = null;
        try {
    ...
    private function parseApiExceptionFromSoapFault(SoapFault $soapFault)
    {
        ...
        if (!array_key_exists($soapFaultDetailName, self::$SOAP_FAULT_NAME_TO_TYPE_MAP)) {
            return false;
        }
        ...
        try {
            $apiException = $this->reflection->createInstance($exceptionClassName, $soapFaultDetail->message);
    ...
    {
        $summary = $this->soapLogMessageFormatter->formatSummary($this->serviceDescriptor->getServiceName(), $methodName, $this->__getLastRequestHeaders(), $this->__getLastRequest(), $this->__getLastResponse(), $soapFault);
        $detailed = $this->soapLogMessageFormatter->formatDetailed($this->__getLastRequestHeaders(), $this->__getLastRequest(), $this->__getLastResponseHeaders(), $this->__getLastResponse());
        if ($soapFault !== null) {
     ...
     */
    public function setSoapLogMessageFormatter(SoapLogMessageFormatter $soapLogMessageFormatter)
    {
     ...
     */
    public function setServiceDescriptor(AdsServiceDescriptor $serviceDescriptor)
    {
    ...
    {
        $resourcesWsdlsPath = sprintf(self::$RESOURCES_WSDLS_PATH_FORMAT, __DIR__, DIRECTORY_SEPARATOR);
        ...
        // WSDLs.
        $wsdlFilePath = str_replace('/', DIRECTORY_SEPARATOR, parse_url($wsdl, PHP_URL_PATH));
