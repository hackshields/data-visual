...
class ExampleTestHelper
{
     ...
     */
    public function __construct(AdManagerServices $services, AdManagerSession $session)
    {
    ...
    public function checkExistingUserTeamAssociation($userId, $teamId)
    {
        ...
        $page = $service->getUserTeamAssociationsByStatement($statementBuilder->toStatement());
    ...
    public function deleteUserTeamAssociation($userId, $teamId)
    {
        ...
        $statementBuilder = (new StatementBuilder())->where('WHERE userId = :userId AND teamId = :teamId')->withBindVariableValue('userId', $userId)->withBindVariableValue('teamId', $teamId);
        $result = $service->performUserTeamAssociationAction(new DeleteUserTeamAssociations(), $statementBuilder->toStatement());
        if (is_null($result) || $result->getNumChanges() < 1) {
            throw new RuntimeException('The requested delete action did not' . ' delete any user-team association records');
        }
    }
    /**
    ...
    private function getUserTeamAssociationService()
    {
        if (is_null($this->userTeamAssociationService)) {
            $this->userTeamAssociationService = $this->services->get($this->session, UserTeamAssociationService::class);
        }
