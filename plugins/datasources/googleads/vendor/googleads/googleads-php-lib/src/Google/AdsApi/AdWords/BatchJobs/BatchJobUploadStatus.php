...
final class BatchJobUploadStatus
{
    ...
    {
        $requestOptions = [];
        $requestOptions[RequestOptions::HEADERS] = ['Content-Type' => 'application/xml', 'Content-Length' => 0, 'x-goog-resumable' => 'start'];
        if (!empty($this->connectionSettings->getProxyUrl())) {
            $requestOptions[RequestOptions::PROXY] = ['https' => $this->connectionSettings->getProxyUrl()];
        }
