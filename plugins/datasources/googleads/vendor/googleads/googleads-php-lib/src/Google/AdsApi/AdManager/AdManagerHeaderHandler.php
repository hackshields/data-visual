 ...
 */
final class AdManagerHeaderHandler implements AdsHeaderHandler
{
    /**
    ...
    public function generateHttpHeaders(AdsSession $session)
    {
        ...
        $httpHeaders = ['Authorization' => sprintf('Bearer %s', urlencode($this->oAuth2TokenRefresher->getOrFetchAccessToken($session->getOAuth2Credential(), $httpHandler)))];
        return $httpHeaders;
    }
    /**
    ...
    public function generateSoapHeaders(AdsSession $session, AdsServiceDescriptor $serviceDescriptor)
    {
        ...
        $soapRequestHeader->setApplicationName($this->formatApplicationNameForSoapHeader($session->getApplicationName()));
    ...
    private function createSoapHeaderObject(AdsServiceDescriptor $serviceDescriptor)
    {
        ...
        return $this->reflection->createInstance(sprintf('%s\\%s', $reflectionClass->getNamespaceName(), self::SOAP_HEADER_CLASS_NAME));
    }
    private function formatApplicationNameForSoapHeader($applicationName)
    {
        return $this->adsHeaderFormatter->formatApplicationNameForSoapHeader($applicationName, self::PRODUCT_NAME_FOR_SOAP_HEADER, false);
    }
    /**
    ...
    {
        return $this->adsHeaderFormatter->scrubHttpHeaders($httpHeaders, $this->httpHeadersToScrub);
    }
    /**
    ...
    {
        return $this->adsHeaderFormatter->scrubSoapHeaders($soapXml, []);
    }
}
