...
class FetchAuthTokenTest extends BaseTest
{
    ...
    {
        $mockFetcher = $this->getMockBuilder($fetcherClass)->disableOriginalConstructor()->getMock();
        $mockFetcher->expects($this->once())->method('fetchAuthToken')->will($this->returnCallback(function ($httpHandler) {
            return $httpHandler();
        }));
        ...
        $client = CredentialsLoader::makeHttpClient($mockFetcher, ['base_url' => 'https://www.googleapis.com/books/v1/', 'base_uri' => 'https://www.googleapis.com/books/v1/', 'exceptions' => false, 'defaults' => ['exceptions' => false]], $httpHandler, $tokenCallback);
        $response = $client->get('volumes?q=Henry+David+Thoreau&country=US');
    ...
    {
        return [['Google\\Auth\\Credentials\\AppIdentityCredentials'], ['Google\\Auth\\Credentials\\GCECredentials'], ['Google\\Auth\\Credentials\\ServiceAccountCredentials'], ['Google\\Auth\\Credentials\\ServiceAccountJwtAccessCredentials'], ['Google\\Auth\\Credentials\\UserRefreshCredentials'], ['Google\\Auth\\OAuth2']];
    }
    public function testAppIdentityCredentialsGetLastReceivedToken()
    {
        $class = new \ReflectionClass('Google\\Auth\\Credentials\\AppIdentityCredentials');
        $property = $class->getProperty('lastReceivedToken');
        $property->setAccessible(true);
        $credentials = new AppIdentityCredentials();
        $property->setValue($credentials, ['access_token' => 'xyz', 'expiration_time' => strtotime('2001')]);
        $this->assertGetLastReceivedToken($credentials);
    }
    public function testGCECredentialsGetLastReceivedToken()
    {
        $class = new \ReflectionClass('Google\\Auth\\Credentials\\GCECredentials');
        $property = $class->getProperty('lastReceivedToken');
        $property->setAccessible(true);
        $credentials = new GCECredentials();
        $property->setValue($credentials, ['access_token' => 'xyz', 'expires_at' => strtotime('2001')]);
        $this->assertGetLastReceivedToken($credentials);
    }
    public function testServiceAccountCredentialsGetLastReceivedToken()
    {
        $jsonPath = sprintf('%s/fixtures/.config/%s', __DIR__, CredentialsLoader::WELL_KNOWN_PATH);
        $class = new \ReflectionClass('Google\\Auth\\Credentials\\ServiceAccountCredentials');
    ...
    {
        $jsonPath = sprintf('%s/fixtures/.config/%s', __DIR__, CredentialsLoader::WELL_KNOWN_PATH);
        $class = new \ReflectionClass('Google\\Auth\\Credentials\\ServiceAccountJwtAccessCredentials');
        $property = $class->getProperty('auth');
        $property->setAccessible(true);
    ...
    {
        $jsonPath = sprintf('%s/fixtures2/.config/%s', __DIR__, CredentialsLoader::WELL_KNOWN_PATH);
    ...
    {
        $oauth = new OAuth2(['access_token' => 'xyz', 'expires_at' => strtotime('2001')]);
        $this->assertGetLastReceivedToken($oauth);
    }
    private function getOAuth2Mock()
    {
        $mock = $this->getMockBuilder('Google\\Auth\\OAuth2')->disableOriginalConstructor()->getMock();
        $mock->expects($this->once())->method('getLastReceivedToken')->will($this->returnValue(['access_token' => 'xyz', 'expires_at' => strtotime('2001')]));
        return $mock;
    }
