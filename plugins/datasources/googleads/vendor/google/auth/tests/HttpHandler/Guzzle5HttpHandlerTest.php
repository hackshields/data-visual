...
class Guzzle5HttpHandlerTest extends BaseTest
{
    ...
    {
        $this->onlyGuzzle5();
        $this->mockPsr7Request = $this->getMockBuilder('Psr\\Http\\Message\\RequestInterface')->getMock();
        $this->mockRequest = $this->getMockBuilder('GuzzleHttp\\Message\\RequestInterface')->getMock();
        $this->mockClient = $this->getMockBuilder('GuzzleHttp\\Client')->disableOriginalConstructor()->getMock();
    ...
    {
        $response = new Response(200, [], Stream::factory('Body Text'));
        $this->mockClient->expects($this->any())->method('send')->will($this->returnValue($response));
        $this->mockClient->expects($this->any())->method('createRequest')->will($this->returnValue($this->mockRequest));
    ...
    public function testAsyncWithoutGuzzlePromiseThrowsException()
    {
        ...
        $this->mockClient->expects($this->any())->method('send')->will($this->returnValue(new FutureResponse($this->mockFuture)));
        $this->mockClient->expects($this->any())->method('createRequest')->will($this->returnValue($this->mockRequest));
        $handler = new Guzzle5HttpHandler($this->mockClient);
        $errorThrown = false;
        try {
            ...
            $this->assertEquals('Install guzzlehttp/promises to use async with Guzzle 5', $e->getMessage());
            $errorThrown = true;
        }
    ...
    {
        $response = new Response(200, [], Stream::factory('Body Text'));
        $this->mockClient->expects($this->any())->method('send')->will($this->returnValue(new FutureResponse(new CompletedFutureValue($response))));
        $this->mockClient->expects($this->any())->method('createRequest')->will($this->returnValue($this->mockRequest));
        $handler = new Guzzle5HttpHandler($this->mockClient);
    ...
    {
        $this->mockClient->expects($this->any())->method('send')->will($this->returnValue(new FutureResponse((new CompletedFutureValue(new Response(200)))->then(function () {
    ...
    public function testCreateGuzzle5Request()
    {
        ...
        $this->mockPsr7Request->expects($this->once())->method('getHeaders')->will($this->returnValue($requestHeaders));
        $mockBody = $this->getMock('Psr\\Http\\Message\\StreamInterface');
        $this->mockPsr7Request->expects($this->once())->method('getBody')->will($this->returnValue($mockBody));
        $this->mockClient->expects($this->once())->method('createRequest')->with(null, null, ['headers' => $requestHeaders + ['header3' => 'value3'], 'body' => $mockBody])->will($this->returnValue($this->getMock('GuzzleHttp\\Message\\RequestInterface')));
        $this->mockClient->expects($this->once())->method('send')->will($this->returnValue($this->getMock('GuzzleHttp\\Message\\ResponseInterface')));
        $handler = new Guzzle5HttpHandler($this->mockClient);
        $handler($this->mockPsr7Request, ['headers' => ['header3' => 'value3']]);
    }
}
