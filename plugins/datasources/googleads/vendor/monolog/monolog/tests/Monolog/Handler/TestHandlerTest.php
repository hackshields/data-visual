...
class TestHandlerTest extends TestCase
{
    ...
    public function testHandlerAssertEmptyContext()
    {
        ...
        $this->assertFalse($handler->hasWarning(array('message' => 'test', 'context' => array())));
        $handler->handle($record);
        $this->assertTrue($handler->hasWarning(array('message' => 'test', 'context' => array())));
        $this->assertFalse($handler->hasWarning(array('message' => 'test', 'context' => array('foo' => 'bar'))));
    }
    public function testHandlerAssertNonEmptyContext()
    {
        ...
        $this->assertFalse($handler->hasWarning(array('message' => 'test', 'context' => array('foo' => 'bar'))));
        $handler->handle($record);
        $this->assertTrue($handler->hasWarning(array('message' => 'test', 'context' => array('foo' => 'bar'))));
        $this->assertFalse($handler->hasWarning(array('message' => 'test', 'context' => array())));
    }
