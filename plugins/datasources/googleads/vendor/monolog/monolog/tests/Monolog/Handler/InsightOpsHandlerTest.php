...
class InsightOpsHandlerTest extends TestCase
{
    ...
    private function createHandler()
    {
        ...
        $this->handler = $this->getMock('\\Monolog\\Handler\\InsightOpsHandler', array('fsockopen', 'streamSetTimeout', 'closeSocket'), $args);
        $reflectionProperty = new \ReflectionProperty('\\Monolog\\Handler\\SocketHandler', 'connectionString');
        $reflectionProperty->setAccessible(true);
        $reflectionProperty->setValue($this->handler, 'localhost:1234');
        $this->handler->expects($this->any())->method('fsockopen')->will($this->returnValue($this->resource));
