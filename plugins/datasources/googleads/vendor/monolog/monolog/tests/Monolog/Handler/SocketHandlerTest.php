...
class SocketHandlerTest extends TestCase
{
    ...
    public function testExceptionIsThrownIfCannotSetChunkSize()
    {
        ...
        $this->handler->expects($this->once())->method('streamSetChunkSize')->will($this->returnValue(false));
        $this->writeRecord('Hello world');
    }
    /**
    ...
    private function setMockHandler(array $methods = array())
    {
        ...
        if (!in_array('streamSetChunkSize', $methods)) {
            $this->handler->expects($this->any())->method('streamSetChunkSize')->will($this->returnValue(8192));
        }
        $this->handler->setFormatter($this->getIdentityFormatter());
    }
}
