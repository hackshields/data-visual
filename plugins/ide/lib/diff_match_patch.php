...
class diff_match_patch
{
    ...
    public function diff_compute($text1, $text2, $checklines)
    {
        ...
        $longtext = mb_strlen($text2) < mb_strlen($text1) ? $text1 : $text2;
        $shorttext = mb_strlen($text2) < mb_strlen($text1) ? $text2 : $text1;
        $i = mb_strpos($longtext, $shorttext);
        if ($i !== false) {
            $diffs = array(array(DIFF_INSERT, mb_substr($longtext, 0, $i)), array(DIFF_EQUAL, $shorttext), array(DIFF_INSERT, mb_substr($longtext, $i + mb_strlen($shorttext))));
        ...
        if ($hm) {
            ...
            return array_merge($diffs_a, array(array(DIFF_EQUAL, $mid_common)), $diffs_b);
        }
        ...
        if ($checklines) {
            ...
            list($text1, $text2, $linearray) = $a;
        }
        $diffs = $this->diff_map($text1, $text2);
        if (!$diffs) {
            $diffs = array(array(DIFF_DELETE, $text1), array(DIFF_INSERT, $text2));
        }
        if ($checklines) {
            ...
            $count_insert = 0;
            $text_delete = "";
            for ($text_insert = ""; $pointer < count($diffs); $pointer++) {
                switch ($diffs[$pointer][0]) {
                    ...
                    case DIFF_DELETE:
                        ...
                        break;
                    case DIFF_EQUAL:
                        if (1 <= $count_delete && 1 <= $count_insert) {
    ...
    public function diff_map($text1, $text2)
    {
        ...
        for ($d = 0; $d < $max_d; $d++) {
            if (0 < $this->Diff_Timeout && $ms_end < microtime(true)) {
                return NULL;
            }
            $v_map1[$d] = array();
            $k = 0 - $d;
            while ($k <= $d) {
                if ($k == 0 - $d || $k != $d && $v1[$k - 1] < $v1[$k + 1]) {
                    $x = $v1[$k + 1];
                } else {
                    $x = $v1[$k - 1] + 1;
                }
            ...
            while (1) {
                if (isset($v_map[$d][$x - 1 . "," . $y])) {
                    ...
                    if ($last_op === DIFF_DELETE) {
                        ...
                        array_unshift($path, array(DIFF_DELETE, mb_substr($text1, $x, 1)));
                    }
                    $last_op = DIFF_DELETE;
                    break;
                }
                if (isset($v_map[$d][$x . "," . ($y - 1)])) {
                    $y--;
                    if ($last_op === DIFF_INSERT) {
                        $path[0][1] = mb_substr($text2, $y, 1) . $path[0][1];
                    } else {
                        array_unshift($path, array(DIFF_INSERT, mb_substr($text2, $y, 1)));
                    }
                    $last_op = DIFF_INSERT;
                    break;
                }
                ...
                if ($last_op === DIFF_EQUAL) {
                    ...
                    array_unshift($path, array(DIFF_EQUAL, mb_substr($text1, $x, 1)));
                }
                $last_op = DIFF_EQUAL;
            }
        }
            ...
            while (1) {
                if (isset($v_map[$d][$x - 1 . "," . $y])) {
                    ...
                    if ($last_op === DIFF_DELETE) {
                        ...
                        $path[$pathLength++] = array(DIFF_DELETE, $text1[mb_strlen($text1) - $x - 1]);
                    }
                    $last_op = DIFF_DELETE;
                    break;
                }
                if (isset($v_map[$d][$x . "," . ($y - 1)])) {
                    ...
                    if ($last_op === DIFF_INSERT) {
                        ...
                        $path[$pathLength++] = array(DIFF_INSERT, $text2[mb_strlen($text2) - $y - 1]);
                    }
                    $last_op = DIFF_INSERT;
                    break;
                }
                ...
                if ($last_op === DIFF_EQUAL) {
                    ...
                    $path[$pathLength++] = array(DIFF_EQUAL, $text1[mb_strlen($text1) - $x - 1]);
                }
                $last_op = DIFF_EQUAL;
            }
        }
    ...
    public function diff_halfMatch($text1, $text2)
    {
        ...
        if (!$hm2) {
            ...
            if (!$hm1) {
                ...
                $hm = mb_strlen($hm2[4]) < mb_strlen($hm1[4]) ? $hm1 : $hm2;
            }
        }
        if (mb_strlen($text2) < mb_strlen($text1)) {
            list($text1_a, $text1_b, $text2_a, $text2_b) = $hm;
        } else {
            list($text2_a, $text2_b, $text1_a, $text1_b) = $hm;
        }
        $mid_common = $hm[4];
    ...
    public function diff_halfMatchI($longtext, $shorttext, $i)
    {
        ...
        if (mb_strlen($longtext) / 2 <= mb_strlen($best_common)) {
    ...
    public function diff_cleanupSemantic(&$diffs)
    {
        ...
        $pointer = 0;
        $length_changes1 = 0;
        for ($length_changes2 = 0; $pointer < count($diffs); $pointer++) {
            if ($diffs[$pointer][0] == DIFF_EQUAL) {
                ...
                if ($lastequality !== NULL && mb_strlen($lastequality) <= $length_changes1 && mb_strlen($lastequality) <= $length_changes2) {
                    $zzz_diffs = array_splice($diffs, $equalities[$equalitiesLength - 1], 0, array(array(DIFF_DELETE, $lastequality)));
                    $diffs[$equalities[$equalitiesLength - 1] + 1][0] = DIFF_INSERT;
                    $equalitiesLength--;
                    $equalitiesLength--;
                    $pointer = 0 < $equalitiesLength ? $equalities[$equalitiesLength - 1] : -1;
                    $length_changes1 = 0;
                    $length_changes2 = 0;
                    $lastequality = NULL;
                    $changes = true;
    ...
    {
        for ($pointer = 1; $pointer < count($diffs) - 1; $pointer++) {
    ...
    public function diff_cleanupEfficiency(&$diffs)
    {
        ...
        $pre_del = false;
        $post_ins = false;
        for ($post_del = false; $pointer < count($diffs); $pointer++) {
            if ($diffs[$pointer][0] == DIFF_EQUAL) {
                ...
                case DIFF_EQUAL:
                    if ($count_delete !== 0 || $count_insert !== 0) {
                        if ($count_delete !== 0 && $count_insert !== 0) {
                            ...
                            if ($commonlength !== 0) {
                                if (0 < $pointer - $count_delete - $count_insert && $diffs[$pointer - $count_delete - $count_insert - 1][0] == DIFF_EQUAL) {
                                    $diffs[$pointer - $count_delete - $count_insert - 1][1] .= mb_substr($text_insert, 0, $commonlength);
                                } else {
                                    array_splice($diffs, 0, 0, array(array(DIFF_EQUAL, mb_substr($text_insert, 0, $commonlength))));
                                    $pointer++;
                                }
                        ...
                        if ($count_delete === 0) {
                            array_splice($diffs, $pointer - $count_delete - $count_insert, $count_delete + $count_insert, array(array(DIFF_INSERT, $text_insert)));
                        } else {
                            if ($count_insert === 0) {
                                array_splice($diffs, $pointer - $count_delete - $count_insert, $count_delete + $count_insert, array(array(DIFF_DELETE, $text_delete)));
                            } else {
        ...
        for ($pointer = 1; $pointer < count($diffs) - 1; $pointer++) {
            if ($diffs[$pointer - 1][0] == DIFF_EQUAL && $diffs[$pointer + 1][0] == DIFF_EQUAL) {
                if (mb_substr($diffs[$pointer][1], mb_strlen($diffs[$pointer][1]) - mb_strlen($diffs[$pointer - 1][1])) == $diffs[$pointer - 1][1]) {
                    ...
                    if (mb_substr($diffs[$pointer][1], 0, mb_strlen($diffs[$pointer + 1][1])) == $diffs[$pointer + 1][1]) {
    ...
    public function diff_fromDelta($text1, $delta)
    {
        ...
        $x = 0;
        while ($x < count($tokens)) {
            $param = mb_substr($tokens[$x], 1);
            switch ($tokens[$x][0]) {
                case "+":
                    try {
                        $diffs[$diffsLength++] = array(DIFF_INSERT, decodeURI($param));
                    } catch (Exception $ex) {
                ...
                case "=":
                    ...
                    $pointer += $n;
                    if ($tokens[$x][0] == "=") {
                        $diffs[$diffsLength++] = array(DIFF_EQUAL, $text);
                    } else {
                        $diffs[$diffsLength++] = array(DIFF_DELETE, $text);
                    }
                    break;
                default:
                    if ($tokens[$x]) {
    ...
    public function match_main($text, $pattern, $loc)
    {
        ...
        if (mb_substr($text, $loc, mb_strlen($pattern)) == $pattern) {
            return $loc;
        }
    ...
    {
        if (Match_MaxBits < mb_strlen($pattern)) {
        ...
        for ($d = 0; $d < mb_strlen($pattern); $d++) {
            ...
            if ($score_threshold < $this->match_bitapScore($d + 1, $loc, $pattern, $loc)) {
                break;
            }
            $last_rd = $rd;
        }
    ...
    public function patch_addContext($patch, $text)
    {
        ...
        $padding = 0;
        $i = 0;
        while ((mb_strlen($pattern) === 0 || mb_strpos($text, $pattern) !== mb_strrpos($text, $pattern)) && $pattern !== $previousPattern && mb_strlen($pattern) < Match_MaxBits - $this->Patch_Margin - $this->Patch_Margin) {
    ...
    public function patch_make($a, $opt_b = NULL, $opt_c = NULL)
    {
        if (is_string($a) && is_string($opt_b) && $opt_c === NULL) {
            ...
            if (is_array($a) && $opt_b === NULL && $opt_c === NULL) {
                $diffs = $a;
                $text1 = $this->diff_text1($diffs);
            } else {
                if (is_string($a) && is_array($opt_b) && $opt_c === NULL) {
                    $text1 = $a;
                    $diffs = $opt_b;
                } else {
                    if (is_string($a) && is_string($opt_b) && is_array($opt_c)) {
                        $text1 = $a;
                        $diffs = $opt_c;
                    } else {
            ...
            switch ($diff_type) {
                ...
                case DIFF_EQUAL:
                    if (mb_strlen($diff_text) <= 2 * $this->Patch_Margin && $patchDiffLength && count($diffs) != $x + 1) {
                        ...
                        if (2 * $this->Patch_Margin <= mb_strlen($diff_text) && $patchDiffLength) {
    ...
    public function patch_apply($patches, $text)
    {
        ...
        for ($x = 0; $x < count($patches); $x++) {
            ...
            $start_loc = NULL;
            $end_loc = -1;
            if (Match_MaxBits < mb_strlen($text1)) {
                ...
                if ($start_loc != -1) {
                    ...
                    if ($end_loc == -1 || $end_loc <= $start_loc) {
                        $start_loc = -1;
                    }
                }
            } else {
            ...
            if ($start_loc == -1) {
                ...
                if ($text1 == $text2) {
                    ...
                    if (Match_MaxBits < mb_strlen($text1) && $this->Patch_DeleteThreshold < $this->diff_levenshtein($diffs) / mb_strlen($text1)) {
                        $results[$x] = false;
                    } else {
                        $this->diff_cleanupSemanticLossless($diffs);
                        $index1 = 0;
    ...
    public function patch_addPadding(&$patches)
    {
        ...
        if (count($diffs) == 0 || $diffs[0][0] != DIFF_EQUAL) {
            ...
            if (mb_strlen($diffs[0][1]) < $paddingLength) {
        ...
        if (count($diffs) == 0 || $diffs[count($diffs) - 1][0] != DIFF_EQUAL) {
            ...
            if (mb_strlen($diffs[count($diffs) - 1][1]) < $paddingLength) {
    ...
    {
        for ($x = 0; $x < count($patches); $x++) {
            if (Match_MaxBits < $patches[$x]->length1) {
                $bigpatch = $patches[$x];
                ...
                while (count($bigpatch->diffs) !== 0) {
                    ...
                    while (count($bigpatch->diffs) !== 0 && $patch->length1 < $patch_size - $this->Patch_Margin) {
                        ...
                        if ($diff_type === DIFF_INSERT) {
                            ...
                            if ($diff_type === DIFF_DELETE && count($patch->diffs) == 1 && $patch->diffs[0][0] == DIFF_EQUAL && 2 * $patch_size < mb_strlen($diff_text)) {
                                $patch->length1 += mb_strlen($diff_text);
...
{
    return strtr(rawurlencode($url), array("%3B" => ";", "%2C" => ",", "%2F" => "/", "%3F" => "?", "%3A" => ":", "%40" => "@", "%26" => "&", "%3D" => "=", "%2B" => "+", "%24" => "\$", "%21" => "!", "%2A" => "*", "%27" => "'", "%28" => "(", "%29" => ")", "%23" => "#"));
}
function decodeURI($encoded)
{
    static $dontDecode = NULL;
    if (!$dontDecode) {
        $table = array("%3B" => ";", "%2C" => ",", "%2F" => "/", "%3F" => "?", "%3A" => ":", "%40" => "@", "%26" => "&", "%3D" => "=", "%2B" => "+", "%24" => "\$", "%21" => "!", "%2A" => "*", "%27" => "'", "%28" => "(", "%29" => ")", "%23" => "#");
        $dontDecode = array();
